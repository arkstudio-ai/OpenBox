...
- **技能与指令：**8 个内置技能按 6 类集中登记；项目、个人库和沙箱技能按作用域加载；支持附属脚本与参考资料、
  项目规则及快捷命令模板。内置和用户添加的技能支持中英文展示名与简介，调用标识保持稳定。
- **可复用 Agent 与团队：**模型/工具/技能配置、定义版本、委派、任务依赖、成员通信、成果验收与
  最终答复。团队花费使用用户账号积分账本。
  每个可复用 Agent 固定具备 10 个核心工具；显式选择技能会自动勾选并锁定依赖的工具和 MCP 服务。
...
backend/
  ...
  skill/ · skill/builtins/      技能服务与分类登记的内置技能包
