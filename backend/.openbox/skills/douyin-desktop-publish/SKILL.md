...
发布、补挂团购、重发、去重或“继续”都不授权删除历史作品。不得自行把删除加入发布计划。
用户明确要求删除时，先列出账号及具体作品的稳定 ID/链接、标题、发布时间、数量和删除影响，
取得对这份清单的明确授权后才可处理；已有同范围授权无需重复索要。
禁止按“第二条”、`deleteButtons[1]` 等动态位置定位；一次工具调用最多处理一个已授权作品，
删除前核对稳定 ID，删除后只读确认。目标已不存在、结果不明或用户质问作品消失时立即停止，
不得继续删除前移补位的作品，不得通过浏览器、Bash、API 或定时任务绕过边界。
