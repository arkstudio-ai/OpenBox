...
class ToolHooks:
    ...
    async def wrap_execute(
        ...
        ctx._trajectory_tool_metadata = (tool_info, provider_call_id, arguments_raw)
        if requested_recorded:
            ctx._trajectory_requested_part = part_id
        return await self._wrap_execute_impl(tool_id, execute_fn, args, ctx, part_id, stream=None)

    async def dispatch_execute(self, prepared: PreparedToolExecution) -> ToolDispatchOutcome:
        if prepared.blocked_result is not None:
            return ToolDispatchOutcome(prepared.blocked_result)
        tool_id, execute_fn, args = prepared.tool_id, prepared.execute_fn, prepared.args
        ctx, part_id = prepared.run_ctx, prepared.part_id
        tool_info = None
        provider_call_id = None
        arguments_raw = None
        requested_recorded = getattr(ctx, "_trajectory_requested_part", None) == part_id
        ...
        with bind(context), _bind_tool_context(ctx):
            ...
            try:
                outcome = await self._dispatch_execute_body(prepared)
                ...
                raise
            if not getattr(execute_fn, "_trajectory_validates", False) and not stream.finished:
                duration = (time.monotonic() - ctx._trajectory_execute_started
                            if ctx._trajectory_execute_started is not None else None)
    ...
    async def prepare_execute(
        self, tool_id: str, execute_fn: Any, args: dict, ctx: ToolContext,
        *, part_id: str = "", isolate_context: bool = False,
        ...
        context = await context_for_tool(ctx)
        if context is not None:
            context = context.derive(call_id=part_id or None, part_id=part_id or None,
                                     message_id=ctx.message_id or context.message_id)
        ctx.trace_context = context
        with bind(context), _bind_tool_context(ctx):
            if getattr(ctx, "_trajectory_requested_part", None) != part_id:
                tool_info, provider_call_id, arguments_raw = getattr(ctx, "_trajectory_tool_metadata", (None, None, None))
                schema, schema_source = requested_tool_schema(ctx, tool_id, tool_info)
                await record("tool.requested", {"tool": tool_id, "provider_call_id": provider_call_id,
                    "arguments_raw": arguments_raw, "requested_arguments": public_value(args),
                    "schema": schema, "schema_source": schema_source}, context=context)
                ctx._trajectory_requested_part = part_id
            prepared = await self._prepare_execute_impl(tool_id, execute_fn, args, ctx,
                part_id=part_id, isolate_context=isolate_context)
            if prepared.blocked_result is not None:
                result = prepared.blocked_result
                await record("tool.finished", {"tool": tool_id, "status": "denied",
                    "title": result.title, "model_output": result.output,
                    "metadata": public_value(result.metadata), "duration_ms": None}, context=context)
            return prepared

    async def _prepare_execute_impl(
        self,
        tool_id: str,
        execute_fn: Any,
        args: dict,
        ...
        *,
        ...
        async def _on_output(output: str) -> None:
            ...
            stream = getattr(run_ctx, "_trajectory_output_stream", None)
    ...
    async def _dispatch_execute_body(
        self,
        ...
        from question.runtime import RunRevoked, assert_current
    ...
    async def finalize_execute(
        ...
        result.metadata.setdefault("duration", duration)
        return result
    ...
    async def authorize_tool(
        ...
        *,
        ...
        if ctx is not None and ctx.sandbox is not None and tool_id in {
