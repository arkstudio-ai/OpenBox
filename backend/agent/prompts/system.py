...
- A slow or paid operation makes this rule matter more, not less: start EVERY independent one before you wait on any of them. Submitting one, waiting for it to finish, then submitting the next multiplies the wall-clock by the number of items and is the single most common way a long task is made needlessly slow. Kick them all off in one response, then poll them together.
