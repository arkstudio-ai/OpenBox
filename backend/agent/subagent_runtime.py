    ...
    project = (
        await db.execute(
            select(Project.id).where(
                ...
                Project.workspace_id == parent.workspace_id,
...
async def accept_spawn(
    *,
    ...
    async with get_db_session() as db:
        ...
        parent_trace = await activity_context(db, user_id, parent_session_id,
            call_id=parent_part_id, part_id=parent_part_id, message_id=parent_message_id)
        child_trace = parent_trace.derive(source_session_id=child_row.id, agent_id=descriptor_id,
            ...
            message_id=None, part_id=None) if parent_trace is not None else None
        ...
        await record("agent.spawned", {"agent": subagent_type, "description": task_title,
            "prompt": prompt, "child_session_id": child_row.id, "model": child_model},
            context=child_trace, db=db, event_id=f"agent_spawn:{activation_id}")
...
async def _complete_locked(
    ...
    *,
    ...
    from trajectory.producers import activity_context
    execution = await db.get(SessionExecution, activation.child_session_id)
    context = await activity_context(db, activation.user_id, activation.child_session_id,
        saved=execution.trace_context if execution is not None else None)
    if context is not None:
        context = context.derive(run_id=None, generation=None, step_id=None, request_id=None,
                                 call_id=None, message_id=None, part_id=None)
        await record("agent.message", {"direction": "child_to_parent",
            "output": projected.get("output", ""), "model_output": projected.get("output", ""),
            "child_session_id": activation.child_session_id}, context=context, db=db,
            event_id=f"agent_result:{activation.id}")
        await record("agent.finished", {"status": "completed" if outcome == OUTCOME_SUCCEEDED else
            "cancelled" if outcome == OUTCOME_INTERRUPTED else "failed",
            "outcome": outcome, "child_session_id": activation.child_session_id,
            "output": projected.get("output", "")}, context=context, db=db,
            event_id=f"agent_finish:{activation.id}")
    return projected
