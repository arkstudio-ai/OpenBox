"""Add user-owned virtual and verified-person video material bindings.
...
def upgrade() -> None:
    ...
    with op.batch_alter_table("video_productions") as batch:
        ...
        batch.add_column(sa.Column("character_identity_id", sa.String(length=64), nullable=True))
...
def downgrade() -> None:
    with op.batch_alter_table("video_productions") as batch:
        batch.drop_constraint("fk_video_productions_character_identity", type_="foreignkey")
    ...
    op.drop_index("ix_video_material_assets_user_updated", table_name="video_material_assets")
    op.drop_table("video_material_assets")
    op.drop_index("ix_video_material_groups_status", table_name="video_material_groups")
    op.drop_index("ix_video_material_groups_user_updated", table_name="video_material_groups")
