...
TASK_BUDGET_DROPIN = "zz-openbox-browser-resources.conf"
...
def task_budget_problems(systemd_root: Path, cgroup: Path) -> list[str]:
    ...
    resource_file = systemd_root / (ACTION_SERVICE + ".d/" + TASK_BUDGET_DROPIN)
    try:
    ...
    # Migrate only the built-in 5G soft / 6G hard pair, not custom policies.
    high, maximum = cgroup / "memory.high", cgroup / "memory.max"
    if high.exists() and maximum.exists() and (
        high.read_text().strip() == str(5 * 1024**3)
        and maximum.read_text().strip() == str(6 * 1024**3)
    ):
        return ["legacy browser memory soft limit can starve the control service"]
    return []
...
def register_boot_service() -> bool:
    ...
    result = subprocess.run(
        ["systemctl", "show", ACTION_SERVICE, "-p", "LoadState", "-p", "MainPID", "-p", "TasksMax", "-p", "MemoryHigh", "-p", "MemoryMax"],
    ...
    # Different drop-in filenames are applied lexically, across directories.
    # Legacy media.conf pins 512 and overrides both browser-resources.conf and
    # the 50-TasksMax.conf generated by set-property during a daemon-reload.
    resources = SYSTEMD_ROOT / (ACTION_SERVICE + ".d/" + TASK_BUDGET_DROPIN)
    # Preserve the hard cap and all custom operator memory policies.
    migrate_memory = properties.get("MemoryMax") == str(6 * 1024**3) and properties.get("MemoryHigh") in {
        str(5 * 1024**3), str(6 * 1024**3)
    }
    desired = {
        unit: SERVICE,
        dropin: "[Unit]\nRequires=openbox-browser-runtime.service\nAfter=openbox-browser-runtime.service\n",
        resources: f"[Service]\nTasksMax={target}\n" + ("MemoryHigh=6G\n" if migrate_memory else ""),
    }
    changed = False
    backup = None
    ...
    if int(properties.get("MainPID", "0")) > 0 and (
        ...
        # Back up the existing property and persist the live update. The
        # lexically late drop-in also survives the reload implicit in enable;
        # set-property alone cannot override a later media.conf permanently.
        # Never change MemoryMax or restart the service/browser.
        ...
        changed = True
    if migrate_memory and properties.get("MemoryHigh") != str(6 * 1024**3):
        subprocess.run(["systemctl", "set-property", ACTION_SERVICE, "MemoryHigh=6G"], check=True, timeout=20)
        changed = True
