...
async def prepare_agent_event_write(
    ...
    *,
    ...
    return await _lock_fenced(db, session_id, user_id)
...
async def repair_canonical_tail_locked(
    ...
    *,
    ...
    # assistant steps stay in the same repair group across those runs.
    identities_by_turn = {}
    for identity in message_turn.values():
        identities_by_turn.setdefault(identity[2], identity)
    if current_trigger_message_id is not None and current_identity is not None and not allow_unanchored_assistant:
        anchor = message_turn.get(current_trigger_message_id)
        if anchor is not None:
            identities_by_turn[anchor[2]] = (*current_identity, anchor[2])
    ...
    for users, assistants, logical in groups:
        ...
        latest_user_parented = bool(
            ...
            or (logical is None and last_assistant is not None
                and not last_assistant.get("parent_id"))
            or (
