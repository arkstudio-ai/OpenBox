...
def isolate_question_lifecycle(monkeypatch):
    from question import runtime

    async def start(session_id, user_id, **kwargs):
        return runtime.RunTicket(session_id, user_id, 0, kwargs["driver_lease"].run_id)
    ...
    monkeypatch.setattr(runtime, "finish_run", no_op)
    monkeypatch.setattr(runtime, "get_run_trace", no_op)
    monkeypatch.setattr(runtime, "heartbeat", heartbeat)
