...
TOOLS = [
    ("team_propose", Proposal, propose, "Propose a bounded team lineup for explicit user confirmation. Required top-level fields: title, goal. For an inline lineup use {title, goal, team: {name, coordinator, preset_members, policy}}. Put all limits and grants inside team.policy. A fixed roster containing inline members uses member_selection=explicit_only and member_creation=run_scoped. Set each member's model_override when the user specifies a model for all members. This suspends until the user responds; it does not start work by itself."),
