...
async def execute_generate(args: VideoGenerateArgs, ctx: ToolContext) -> ToolResult:
    ...
    if args.action == "submit":
        ...
        try:
            ...
            lines = _job_lines(job, asset)
            if approved.get("dropped"):
                # Capability degradation must remain visible to the caller.
...
async def _finalize_transcription(
    ...
    *,
    ...
    audio = source_asset
    if audio is None:
        if getattr(job, "output_asset_id", None) or getattr(job, "segment_id", None):
            audio = await _job_asset(job)
        else:
            source_asset_id = (getattr(job, "request_data", None) or {}).get(
                "source_asset_id"
            )
            audio = await _find_owned_asset(source_asset_id or "", ctx)
    head = await oss.head(audio.oss_key) if audio else None
    if persist_guard is not None:
        await persist_guard()
    if not audio or not head:
        error = "the owned audio asset is missing from OSS"
        if getattr(job, "output_asset_id", None):
    ...
    stt_effect = (
        dict(existing_result.get("stt_effect"))
        if isinstance(existing_result.get("stt_effect"), dict)
        else {}
    )
    if transcript is None:
        operation_key = str(
            stt_effect.get("operation_key")
            or f"openbox:stt:{job.id}:{job.request_hash[:16]}"
        )
        task_id = str(job.provider_task_id or stt_effect.get("task_id") or "")
        stt_effect = {
            "operation_key": operation_key,
        ...
        async with get_db_session() as db:
            ...
            if claimed.rowcount != 1:
                return await _owned_job(job.id, ctx, "stt")
        try:
            await ctx.update_output("Running speech-to-text…")
    ...
    if not getattr(job, "segment_id", None):
        if persist_guard is not None:
            await persist_guard()
        completed_effect = dict(stt_effect)
        completed_effect["state"] = "completed"
        await _update_job(
            job.id,
            ctx=ctx if persist_guard is not None else None,
...
async def execute_transcribe(args: VideoTranscribeArgs, ctx: ToolContext) -> ToolResult:
    ...
    if args.action == "submit":
        ...
        try:
            ...
            job = await _finalize_transcription(
                ...
                target,
                oss,
                {"source_asset_id": source.id, "source_bytes": source.size},
                persist_guard=_ctx_persist_guard(ctx),
                source_asset=source,
            )
            transcript = (
                (job.result_data or {}).get("transcript")
                if isinstance((job.result_data or {}).get("transcript"), dict)
                else {}
            )
            return ToolResult(
                title=(
            ...
            if isinstance(exc, LeaseLostError):
                raise
            if created and "job" in locals():
                current = await _owned_job(job.id, ctx, "stt")
                if current and current.status == "dispatching":
                    await _update_job(
                        job.id,
                        ctx=ctx,
    ...
    if args.action == "retry":
        ...
        try:
            ...
            if not changed:
                raise RuntimeError("transcription retry ownership was lost")
            job = await _owned_job(job.id, ctx, "stt")
            job = await _finalize_transcription(
                job,
                ctx,
    ...
    if args.action in {"status", "wait"} and job.status in {
        "dispatching",
        "transcribing",
        "transcript_ready",
    }:
