...
> 状态：设计计划已编写；本文描述的新增能力尚待实施，不代表已经上线。
>
> 日期：2026-09-20。
>
...
Web frontend-v2 完成配置与运行管理。移动端支持在输入区选团队模式与已保存的团队模板发起、查看进度与成果、回答问题、暂停与取消；Agent 与模板的编辑通过 Web 完成，界面应明确入口。组队方案卡和回答问题都不需要移动端新做界面：它们是根会话里的提问卡片，走的是移动端已经支持的提问卡片和推送（第 7.6 节），移动端未实现专用详情之前，阵容以文字列在问题正文里。移动端的运行视图是成员列表加详情，不画连线图；不提供成员会话的逐步查看，成员的工具细节在手机上不便查看，与移动端不做会话追踪页面的既有取舍一致。
