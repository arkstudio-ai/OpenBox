...
| 栈 | `/opt/openbox` 与 gw2 同构但无 trajectory overlay；镜像 backend `20260922-gaode-bf982a6` / frontend `20260922-gaode-8cf388f`（本分支；前端含 `/v1` nginx 路由，后端含 `/v1` 跨域放行）；`BILLING_MODE=enforce`、`WUYING_ENV_TAG=gaode`、`POOL_ENABLED=true POOL_AUTO_PURCHASE=false`、`RATE_LIMIT_API=60/minute` |
