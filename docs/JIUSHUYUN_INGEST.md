...
## 3.5 D1 实测结果（2026-09-19，首商家"泽岚鲜果"）

**已完成**
- 来客侧授权：商户 `7670896791667230747`（店长李洁如），SaaS系统授权 → 帆软软件有限公司 / E数通 / 到店餐饮解决方案 / 全部功能（28 项）。这一步是**永久的，每个商户只做一次**。
- 九数云连接：名称 `lk_123`（命名未按约定，待改），connectId `9340d02d9a594cacbf51b427ba93cae5`，商家id `7670896791667230747`，授权时间 2026-09-19 15:08:41。
- 12 张表全部同步成功，共 71 行：订单查询 15、联系人 15、子单金额 15、商品 15、履约 7、退款单 1、售后子单 1、账单详情 1、门店信息 1、评价 0、评价图片 0、次卡退款 0。
- 12 张表已通过「添加到项目」进入 `openbox-ingest`（folderId `agqliy7uwfzy5hoqmja7ydj43m`）。
- **导出接口实测打通**（见下）。

**导出链路（与原设计不同，按此实现）**
`POST /api/v2/export` 是**异步**的：
1. 首次调用返回 `{"errorCode":"61311022","errorMsg":"table exporting,try later..."}`，仅触发生成；
2. 约 3 秒后重复调用，返回 `{"success":true,"data":"<CSV 预签名 URL>"}`，域名 `qfx30.jiushuyun.com`，路径含日期，带 `response-content-disposition` 签名参数；
3. 该 URL **免鉴权** GET 即可拿到 CSV（UTF-8、全字段双引号）。
→ `sync.py` 必须实现"触发 + 轮询 + 取 URL + 下载"四步，不能当成同步接口。

**CSV 实际字段（订单查询，20 列）**
`抖音订单ID, 抖音用户id, 订单实付金额, 折扣金额, 订单创建时间, 意向门店ID, 意向门店名称, 门店名称, 门店ID, 商家账号, 商家名称, 订单支付时间, 最新修改时间, 订单状态, 原始金额, 金额信息_原始金额, 金额信息_订单实付金额, 订单类型, connectId（系统字段）, hashId（系统字段）`

要点：
- 列名是**中文显示名**，不是字段清单里的英文原始名。解析按中文列名映射。
- 九数云自动追加 `connectId（系统字段）` 和 `hashId（系统字段）` 两列。**`connectId` 正是多商户区分键**，落地表直接用它，不必额外传参。
- **金额带千分位逗号**（`"4,500"`），单位仍是分。解析必须先去逗号再转数字。
- 时间已格式化为 `YYYY-MM-DD HH:MM:SS`（东八区），不是时间戳。
- 空值为空字符串；本商户 `门店ID`/`门店名称` 为空，只有 `意向门店ID`，做门店维度要用 `门店管理-查询门店信息` 表补。

**定时同步：九数云硬性建议单日不超过 5 次**
设置弹窗原文："建议单日定时同步不超过 5 次，频率过高会出现以下问题：1.上游数据限流，一段时间内无法同步 2.数据一直在同步，导致加载失败"。
→ **原设计的"每小时同步"作废**。已设为每天 5 次：**6点 / 11点 / 16点 / 20点 / 23点**。
→ 数据新鲜度上限变为约 4 小时，§5.5 的 openbox 拉取任务相应改为每天 5 次、在上述时点后 15 分钟运行；§5.6 的"停更 3 小时告警"改为"超过 6 小时未更新"。

**授权流程的真实形态（§4.1 按此更新）**
1. 在九数云点「抖音来客 → 新增连接」，表单其实是 `dm.shukuajing.com` 的跨域 iframe，其 `identify` JWT **只有 300 秒有效期**。
2. 点「点击授权」触发 `POST dm.shukuajing.com/sync/datasource/sync/credentials/{id}/auth/consent/url`，返回 `https://auth.dylk.com/auth-isv/?client_key=aw4q2o8swl72e5mb&solution_key=1&permission_keys=…&extra=<uuid>`。
3. 该链接**可独立打开、可跨设备**，发给商户即可；商户登录来客、点「授权绑定」后，运营侧表单自动变「已完成」。
4. **弹窗每次都要求商户登录**，抖音侧的持久授权不会让它免登。
5. 「已完成」后必须**立刻**走完「下一步 → 全选 → 确认」，否则 identify 过期，确认会白屏失败（09-19 已踩，导致商户重复授权一次）。

→ 运营 SOP：**先约好商户在线**，运营点授权拿链接 → 商户 1 分钟内完成 → 运营立即建连。不要异步发链接等商户慢慢点。

**待办**
- `lk_123` 改名为 `lk_{workspace_id}` 约定名；另有一条早前失败留下的空连接 `lk_7670896791667230747` 需删除。
- 账单详情表只有 1 行（15 单对应），需确认是商户本身结算少，还是接口只返回指定账单日期。
