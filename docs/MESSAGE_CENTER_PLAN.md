...
- **M2 Web 用户侧：已完成（2026-09-11）**，见 [MESSAGE_CENTER.md](MESSAGE_CENTER.md)「Web 用户侧」。PR #20 早于本分支基线合入，铃铛直接放在合并后的侧栏。与本方案的差异：传输层放在 `shared/api/inbox.ts`（侧栏属 workspace feature，不能跨 feature 引用）；授权中心条幅未切新接口（定位不同，见文档）。
- M5 上线：未开始。
