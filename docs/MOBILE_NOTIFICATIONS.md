...
超管在设置 → 账号 → 系统通知中仅看到通知开关和“系统设置”按钮；不展示连接状态、说明文案、重试按钮或常驻错误文字。开关操作失败时通过现有 Toast 提示。普通用户和工作空间 owner 不显示该卡片，通知默认开启；首次登录且 App 处于前台时申请系统授权，既有拒绝或明确关闭选择不会被覆盖。Android 沿用旧版极光 SDK 的同意流程。启动、登录、回前台和 token 更新时同步绑定，拒绝通知不影响手机登录互踢。
