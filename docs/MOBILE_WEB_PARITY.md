...
>
> 最近核对与实现：2026-09-10；最近核对的 main 基线 `a69b141`。本轮在 `codex/mobile-fixes` worktree 补齐云电脑登录态、授权通知、HTTPS 技能图标和多压缩包上传队列，详见 §11；本轮新增代码尚未提交或发布。阿里云配置为 `APP_ENV=prod`。
...
最初 `2c64409..49ba4ff` 的 8 个 Web 提交已按下表处理。本次追加核对 `205226d..a69b141` 的 18 个 `frontend-v2/src` 提交：Ask 持久化/重连/分页、视频交付排序/折叠、媒体用量账单和技能商店来源/审核/撤回已经同步，未重复实现；本轮补齐云电脑站点登录管理、通知、技能图片图标和批量上传，并修复 locale 与文件大小门禁（见 §11）。超管系统继续有意省略。正式商店发布仍须完成既有的真机支付实测与 iOS 合规决策。
...
| P1 | 云电脑站点登录态、登录页跳转与失效通知 | `0365150`、`7c891ee` | 已完成（代码、单测与 iOS 模拟器联调） | 显式查询 desktop 平台，OAuth/cookie 分离、轮询、检测、确认退出、预计到期、通知已读；第三方扫码成功与真实退出待人工验收，见 §11 |
| P2 | HTTPS 技能图标与多压缩包上传队列 | `d445b9f` | 已完成（代码与单测）；原生选择器已模拟器验收 | 图片失败回退、串行流式上传、部分失败重试、去重和容量限制；未向生产上传测试技能，见 §11 |
...
原先移动端有意省略舰队运维入口。2026-09-10 用户明确要求移动运营后，已补齐原生控制台，详见 §12；仅平台 admin 可访问，普通 workspace owner 不获得超管权限。
...
Web `/app/admin` 下的三个栏目（舰队管理 / 技能管理 / 订阅管理）全部只对 `users.role === "admin"` 开放，后端也只认这一个全局角色。2026-09-08 当时移动端不加入口；2026-09-10 用户明确要求实现并测试后，已改为完整原生入口与移动重排（§12）。`admin.json`、`admin-skills.json`、`admin-billing.json` 继续以 Web 为唯一文案源，并逐字节同步。
...
## 12. 移动端超管控制台（2026-09-10）

用户明确要求实现、在 iOS 模拟器测试并优化手机交互，替代 §5 / §10 的历史 Web-only 决策。本轮仍位于 `codex/mobile-fixes` worktree，基线 `a69b141`，未提交、未推送、未部署。
...
- UI 实现由 Codex CLI 完成，组件预览使用隔离数据，输出在 `mobile/build/admin-ui/`。真实 iOS 验收发现搜索栏在键盘切换时被重建，反馈给同一 CLI 会话后补回归、修复并重新验收；没有提交、推送或部署。
...
5. **Fleet**：已按明确移动运营要求补齐原生入口与操作，见 §12。
...
12. **超管系统 + 技能商店**：移动超管三大模块已实现并重排手机交互，取代历史 Web-only 决策，见 §12。用户侧来源分区/审核文案见 §10，HTTPS 图标与批量压缩包上传见 §11。
13. **云电脑登录态 + 通知**：移动端已补齐站点登录、检测、轮询、确认退出、预计到期与通知已读；iOS 模拟器实连生产验证登录页跳转、等待状态及通知，见 §11。

## 完成记录

| 日期 | 功能块 | 提交 | 验收证据 |
|---|---|---|---|
| 2026-09-10 | Codex CLI 移动 UI 重构与键盘焦点修复 | 本轮 worktree，基线 `a69b141` | 246 项测试、静态分析、翻译/800 行门禁、双端 debug 构建；iOS 26.5 真实键盘、表单、超管与只读查询验收，详见 §12 |
| 2026-09-10 | 移动超管：舰队 / 技能 / 订阅 | 本轮 worktree，基线 `a69b141` | 230 项 Flutter 测试、静态分析、locale/800 行门禁、双端构建；iOS 实连生产验证菜单与只读流程，危险写操作隔离测试，详见 §12 |
| 2026-09-10 | 云电脑登录态、通知、HTTPS 技能图标与批量上传 | 本轮 worktree，基线 `a69b141` | 194 项 Flutter 测试、analyze、locale/800 行门禁、Android/iOS 构建；iOS 26.5 实连生产检查登录页跳转、通知已读与原生文件选择器，完整边界见 §11 |
