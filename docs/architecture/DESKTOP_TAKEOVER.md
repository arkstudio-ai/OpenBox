...
- `backend/tool/desktop/desktop_takeover.py`：新工具。参数 `reason`（captcha_slider / captcha_click /
...
- 提示词：`agent/prompts/system.py` 的 `_INTERNET_AND_BROWSER` 新增「Captcha / risk control: hand off
  to the user」；`tool/knowledge/skill_tool.py` 的 `<browser_mode>` 块在 local/extension 两个分支各加一句。
...
- `backend/sandbox/browser_runtime_repair.py` 的 `RUNTIME_VERSION` 随合并 bump 到 `20260910.2`（现网桌面在 andrew 的 `20260910.1`），
  发布后需按 `docs/operations/DEPLOY.md` 的浏览器运行时修复流程下发到现网桌面，否则旧桌面上的 SKILL.md 不会更新。
