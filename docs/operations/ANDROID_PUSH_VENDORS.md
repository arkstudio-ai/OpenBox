...
厂商 SDK、极光适配器、通知点击解析和正式签名入口已接好。小米通道已使用 2026-09-18 从极光控制台导出的 SDK 配置启用；其他厂商缺少配置时不阻塞开发构建，补齐参数后构建自动包含对应通道。无需再修改业务通知、设备绑定或 Flutter 页面。

沿用应用 `com.bossip.bipmobile` 和极光 AppKey `20c609b5064f10d52d8351d8`。JPush 6.2.1、JCore 5.5.1，七个官方厂商适配器均为 6.2.1。构建仓库、传递依赖和混淆规则已配置。导出包内 JPush、JCore、小米适配器和 MiPush 7.12.4-C 的类文件与当前 Maven 依赖一致，因此继续使用带完整 Manifest、资源和传递依赖的 Maven AAR，避免重复打包同一批类。[极光厂商接入文档](https://docs.jiguang.cn/jpush/client/Android/android_3rd_guide)、[Android SDK 更新记录](https://docs.jiguang.cn/jpush/jpush_changelog/updates_Android)。

## 你需要填写的配置

1. 小米客户端标识已保存在 `mobile/android/push-vendors.client.properties`，普通 Android 构建默认包含小米通道。要增加其他厂商或覆盖小米配置，将 `mobile/android/push-vendors.properties.example` 复制为同目录 `push-vendors.properties`；不使用的厂商留空。
...
| 小米 | 已配置；可用 `XIAOMI_APPID`、`XIAOMI_APPKEY` 覆盖 | 小米 SDK、极光小米适配器及 Manifest 元数据 |
...
CI 可用 `BOSSIP_PUSH_<属性名>` 环境变量覆盖属性文件，例如 `BOSSIP_PUSH_XIAOMI_APPID`。覆盖小米时应同时提供 AppID 和 AppKey。优先级为环境变量、忽略的本地属性文件、仓库内客户端属性文件。构建日志仅输出启用的通道名称，不输出参数值。
...
忽略的覆盖属性文件、JSON 文件、签名文件均排除 Git。仓库内的小米 AppID/AppKey 会随 Manifest 打入 APK，属于客户端可见标识；小米 AppSecret 未写入仓库。OPPO `APPSECRET` 是其客户端适配器所需参数；极光 Master Secret、厂商服务端密钥和 FCM 服务账号私钥应放在后端或极光控制台，不能替代客户端配置文件。
...
SDK 在用户同意通知说明之后初始化，华为和 FCM 的推送自动初始化均保持关闭，由极光统一启用。App 前台或仍可见但失焦时不发送系统通知；后台稳定 3 秒后允许，连续 90 秒无有效上报推断离线。完整边界见 [移动通知说明](../reference/MOBILE_NOTIFICATIONS.md)。
...
2026-09-18，使用极光控制台导出的 `jpush-android-6.2.1-release.zip`（SHA-256 `65170452baf720d49e388b49201ab5926c7d8096a24f8813085f94c6395d830d`）接入小米客户端配置。默认 debug APK 构建通过，`BuildConfig.PUSH_VENDORS` 为 `xiaomi`；合并 Manifest 中 JPush 接收器、BossIP 桥接服务、MiPush 服务与点击 Activity 均存在，小米元数据与导出包一致，且未混入未配置的其他厂商组件。
