...
2026-09-07 更新：付费套餐与无影云自动开通、Free 普通对话、到期停用但保留机器的规则见 [订阅 Sandbox 生命周期](../architecture/SUBSCRIPTION_SANDBOX.md)。该实现不改变本文的模型积分计量；云电脑不再需要用户先手动开通。

2026-09-18 恢复正式价格：按用户要求，默认 `billing/plans.json` 已恢复提交 `9b298e04` 确认的价格：Pro **¥499/月、¥5,988/年**，Max **¥1,999/月、¥23,988/年**。Free、积分额度、有效期及额外充值规则不变。生产环境应清空 `BILLING_PLANS_FILE` 并重启后端；`billing/plans.payment-test.json` 仅供显式启用的支付测试，仍保留 ¥0.10。价格改动只影响新订单，既有订单、付款链接及已购订阅保留原快照。

历史记录：2026-09-07 曾将默认目录和支付测试目录的 Pro / Max 月付、年付总价临时统一为 ¥0.10；该测试价不改变阿里云基础设施成本，支付成功仍触发真实无影云开通。离线浏览器计费回归可在 `frontend-v2` 运行 `npx playwright test --config playwright.billing.config.ts`，使用仓库套餐目录和模拟支付接口，不创建真实支付订单。
...
商业参数参考邻接 workspace 的 bossip 套餐目录（外部参考：`bossip/apps/center/src/billing/plans.config.yaml`）；积分发放参考 SubscriptionService（外部参考：`bossip/apps/center/src/billing/subscription.service.ts`）。采用仓库中的默认配置，不读取 bossip 线上数据库覆盖值。OpenBox 的统一配置为 `backend/billing/plans.json`，可通过 `BILLING_PLANS_FILE` 指定完整替代配置并重启。这里只接入价格、积分和账期，不把另一个产品的云桌面、视频或席位权益作为已经实现的套餐承诺。

| 套餐 | 月付 | 年付 | 周期积分 |
| --- | ---: | ---: | --- |
| 免费版 | ¥0 | ¥0 | 每周 10 |
| 专业版 | ¥499 | ¥5,988 | 每月 280 |
| 旗舰版 | ¥1,999 | ¥23,988 | 每月 1,680 |
...
enforce 走 `post_ledger` 扣积分，shadow 只记录。视频生成与转写仍未落账。详见 `docs/research/VIDEO_RENDER_ENGINE_SELECTION.md` §5.5。
