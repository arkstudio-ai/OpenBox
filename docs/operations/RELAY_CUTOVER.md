...
> 初期开发和部署麻烦一点可以接受。<br>
> 姊妹文档：`docs/plans/product/BILLING_PLAN.md`（计费方案，§5 网关侧准备指向本文）。
...
| 每用户 token | openbox 后端调 `POST /api/token/`，头 `Authorization: Bearer <openbox access token>` + `New-Api-User: <openbox 用户 id>`；name = openbox user id，group `openbox`，model_limits 按套餐 | 与 `docs/plans/product/BILLING_PLAN.md` §4.4 影子上限一致 |
