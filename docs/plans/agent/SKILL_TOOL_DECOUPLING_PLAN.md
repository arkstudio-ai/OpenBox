...
> 姊妹文档：`docs/plans/agent/DIRECT_PATH_CLEANUP_PLAN.md`（上一轮清理的手册，环境事实与坑
...
1. 本文件全文；
2. `docs/plans/agent/DIRECT_PATH_CLEANUP_PLAN.md` §0.3 环境地图与 §8 坑清单（端口 8080、
...
7. **`skill/skill.py:24`**：`allowed_tools` 字段**保留**（技能中心列表展示还在用它
   ...
   解耦，见 docs/plans/agent/SKILL_TOOL_DECOUPLING_PLAN.md）"。`normalize_skill_tools` 保留。
...
1. **分组 stub 按需展开机制（B2）**：实测全量 build 工具定义仍达到约 56.5K 字符，
   已独立立项；完整执行手册见 `docs/plans/agent/TOOL_SCHEMA_DEFERRED_LOADING_PLAN.md`。本手册的
