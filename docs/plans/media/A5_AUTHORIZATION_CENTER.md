...
> 2026-09-07。自包含执行单。取代 `docs/plans/product/DETAILED_PLAN_M1_M2.md` §A5 中"桌面 cookie 探活"的 v0 方案（见 §7 关系说明）。
...
5. `docs/reference/API_INTERFACES.md` 补条目；`DETAILED_PLAN_M1_M2.md` 进度表 A5 改为"进行中/已验收"。
...
| 没有通知模型（A5 v2 已指出） | `docs/plans/product/DETAILED_PLAN_M1_M2.md` §A5 |
...
2. ✅ 2026-09-07 16:00 已合 main（`a043dea`，迁移改接 `a4b6c8d0e2f5` 之后）并以 `20260907-a5-a043dea` 先后部署 AWS 与 gw2；gw2 库升到 `a5c0d1e2f3a4`，备份 `backups/pre-a5-20260907155958.sql.gz`。公网已验：Webhook challenge、错签名 401、回调 302、前端含 auth-center 路由。详见 `docs/operations/DEPLOY.md` 2026-09-07 条目。
...
基于 main `205226d` 对齐 A5 P0/P1、预计到期/短链修正及 P2 聊天产物。完整清单与证据见 [`MOBILE_WEB_PARITY.md` §9](../../reports/mobile/MOBILE_WEB_PARITY.md#9-a5-授权中心抖音投稿与聊天二维码2026-09-07)。
