...
> 2026-09-08。自包含执行单。承接 `docs/plans/media/A5_AUTHORIZATION_CENTER.md`（开放平台 OAuth 路线已上线）与 `docs/plans/product/DETAILED_PLAN_M1_M2.md` §A5 v0（桌面 cookie 探活的原始设计）、`DETAILED_PLAN_M1_M2_REVIEW.md` §2.4（CDP 坑）、`docs/plans/platform/D1_ECD_BROWSER_DEBUG.md`（桌面浏览器诊断底座）。
> 状态：**计划，未开工**。
>
...
Codex 按 `docs/reports/platform/A5_VERIFY_CHECKLIST.md` 验收，抓到两处由我引入的回归，当晚修掉：
...
上线：AWS backend + frontend `20260908-a5fix-7c891ee`；gw2 backend `20260908-a5fix-7c891ee`，frontend 仍是队友钉的 landing（旧前端现在能正常打开授权中心）。Codex 的验收记录与证据在 `docs/reports/platform/A5_VERIFY_CHECKLIST.md` 末尾和 `docs/evidence/a5-verify-20260908/`；其余未验项（扫码发布、退出重登、普通成员、模型 3.1–3.5）待用户配合后重跑。
