...
**另一半是硬闸机制**（gate.py 八相位状态机、exit 非零、contentHash 作废确认），这半 openbox 是**有意退役**的（`docs/plans/media/VIDEO_ATOMIZATION_PLAN.md` §1.6：花费护栏走积分/额度不做审批卡，脚本「只助不阻」），不能搬。
...
## 2.7 用户体验要求（2026-09-04 用户实测后拍板，优先级高于 §2 的逐条对照）

用户按现有技能跑了一条片，体验不好。下面每条都是硬要求，落地位置写在括号里；与 §2 重复的以本节为准。

| # | 要求 | 落地 |
|---|---|---|
| U1 | **成稿确认卡**：讲稿全文贴在卡前；卡上带时长选项——用户没给时长时按默认 45–60 秒先出稿，选项为「可以 / 短到约 30 秒 / 长到 60–75 秒 / 需要修改」，**不单独先问时长**（用户手里没稿答不了）；用户给了时长就按给的。同一张卡问「要不要字幕」（默认配字幕） | SKILL.md Workflow 步骤 2 |
| U2 | **拆段确认卡（含费用）**：成稿确认后进拆段，拆完**必须**出卡，**不许直接提交**。卡前完整贴出：每段台词、每段发给模型的 **prompt 原文（一字不少）**、每段秒数、素材清单（哪张图 / 哪段视频、用在哪几段）、模型与分辨率、按这稿的总时长（与用户要求不符时明说「约 39 秒，要不要删」）、费用报价（段数、总秒数、`estimate` 校验结果）。费用不再单独出卡。没点「可以」一次 `submit` 都不许发 | SKILL.md 步骤 3–7 合并成一张卡；`state.py confirm --kind shots` 记 hash |
| U3 | **删除「无锚图就自动 `image_gen`」**。规则改为：优先用用户提供的形象素材（图或视频）；零素材用文字画面基底（各段一字不差）；**只有用户明确要求才生成参考图**。素材被模型拒绝（如 720p SD 档丢视频参考、工具层直接拒）时**告诉用户**换模型或换素材，不得自作主张改生成锚图。U2 的素材清单就是这次校验 | SKILL.md 步骤 4 重写；`model-guide.md` 注明 720p 丢视频参考 |
| U4 | **中途改稿链**：讲稿改了 → 拆段与 prompt 作废，重新拆、重新出 U2 卡；拆段改了 → 费用作废；已生成且未受影响的段保留，只重做受影响段。`state.py check` 提示「已确认内容已改动」 | `state.py confirm/check` 三级 hash：script → shots → spend |
| U5 | **STT 对不上要提醒**：每段贴可播放链接 + 想说的 + 实际念出 + 判定；相似度低或有替换词标红；**段实际时长与计划秒数偏差超过 max(2s, 25%) 也算对不上**。由用户决定重生或接受（接受记 `--accept 理由`） | SKILL.md 步骤 8；`state.py check` |
...
### 3.1 目标
把 §2.7 的 U1–U8 与 §2 的 24 条「采纳」落进 `backend/.openbox/skills/video-production/`（冲突时 §2.7 优先），每关实拍一条 15 秒口播验证；不改后端、前端、工具层；`state.py` 与 `lint_prompt.py` 保持退出 0。
...
| 文件 | 改动 |
|---|---|
| `SKILL.md` | 新增「硬规则」段（素材外传红线 U7、必停点纪律 #2/#3/#4、交付走 `share_file` U6、不暴露内部信息 #37、不自动生成锚图 U3）；Workflow 改为**三张 `question` 卡**：成稿卡（含时长选项与字幕选项，U1）→ 拆段卡（含 prompt 原文、素材清单、总时长、费用，U2）→ STT 结果卡（含链接与时长偏差，U5）；成片形式不再单独问（已在 U1 问过字幕）。加改稿链 U4、样片先行 #6、续跑先读 state #7、计划≠审批 #8、每段提交即记录 #25、失败原样重试一次 #26；Pitfalls 扩到 13 条 #40；结尾投稿占位 #36。保持「craft not pipeline」的语气：卡是问法，不是闸 |
...
| `scripts/state.py` | 新增 `confirm --kind script\|shots --note`（记 hash，shots 确认隐含费用确认 U2）、`shot --accept`、`check [--final]`（缺项清单：script/shots hash 漂移与受影响段号 U4、段无 job、段无音轨、段实际时长与计划偏差超 max(2s,25%) U5、成片时长≠Σ段、成片无音轨；**全部只打印，退出 0**）；配单测 |
...
| AC-2 | 必停点实跑 | 一次真实会话：三张 `question` 卡按顺序出现，内容在卡前；成稿卡带时长与字幕选项（U1）；拆段卡带每段 prompt 原文、素材清单、总时长、费用（U2）；点「可以」前会话里没有任何 `submit` |
| AC-2b | 改稿链 | 成稿确认后改一句台词 → agent 重新拆段并重新出拆段卡，只重做受影响段；`state.py check` 报出受影响段号（U4） |
| AC-2c | 素材优先 | 用户上传一段人物视频 + 选 720p SD 模型 → agent 在拆段卡里说明该档丢视频参考并请用户换模型或素材，**不生成锚图**；用户未传素材且未要求 → 不调用 `image_gen`，用文字画面基底（U3） |
| AC-2d | STT 提醒 | 人为造一段替换词与一段时长偏差 → 结果卡两段标红并给出原因（U5） |
