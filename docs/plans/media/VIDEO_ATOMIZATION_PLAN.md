...
> 姊妹文档：`docs/plans/agent/SKILL_TOOL_DECOUPLING_PLAN.md`（下称 DECOUPLE）、
> `docs/plans/agent/DIRECT_PATH_CLEANUP_PLAN.md`（下称 CLEANUP，环境地图 §0.3 与坑清单 §8 全部
> 沿用）、`docs/plans/agent/TOOL_SCHEMA_DEFERRED_LOADING_PLAN.md`（下称 DEFER，INTENT_PACKS
> 改动需与其对齐）。
...
`docs/plans/agent/DIRECT_PATH_CLEANUP_PLAN.md` 记的「dev 桌面已过期不可用」**已失效**：
