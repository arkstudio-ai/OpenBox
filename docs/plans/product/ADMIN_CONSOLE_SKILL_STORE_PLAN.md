...
| 管理端：`users.role ∈ {user, developer, admin}`，`require_admin` 只认 `admin`；角色在 access JWT 里，改库后要重新登录；SSO 首登不带 admin，只能运维改 `users.role` | `backend/auth/middleware.py:75`、`docs/plans/platform/A3_A4_FLEET_POOL.md` §3-7e |
...
| 路线图里程碑三写"第一方技能市场只发布我们自己的技能，不开放第三方投稿"——与本单口径不同，本单以用户 2026-09-08 的要求为准（§3-Q1） | `docs/plans/product/PLAN_SHARE.md:73` |
| 移动端：技能中心（含发布）已同构；舰队管理"有意省略"；locale 文件逐字节复制，有门禁 `mobile/scripts/check_locales.sh` | `mobile/lib/features/skills/**`、`docs/reports/mobile/MOBILE_WEB_PARITY.md` |
...
- `docs/reference/API_INTERFACES.md`：新增「16. Admin 技能管理」「17. Admin 订阅管理」；用户侧 `catalog / config / publish / withdraw` 条目更新。
- `docs/plans/product/DETAILED_PLAN_M1_M2.md` 进度表加一行「超管系统 + 技能商店」。
- `docs/reports/mobile/MOBILE_WEB_PARITY.md`：超管系统「有意省略」；用户侧商店变化「待对齐」。
- `docs/plans/product/PLAN_SHARE.md` 里程碑三第 3 条改口为「官方技能 + 经审核的用户投稿」。
...
8. 移动端只做了 locale 镜像，UI 一行没动，已登记在 `docs/reports/mobile/MOBILE_WEB_PARITY.md` §10。
