...
> 2026-09-05 更新：Token 积分计费与支付适配已实现，当前规则为 **1 元 = 1 积分**。实施口径、价格来源、支付合同及模式开关见 [CREDIT_BILLING.md](../../operations/CREDIT_BILLING.md)。下文保留为历史设计，其换算比例不再适用。
...
> 姊妹文档：`docs/operations/WUYING_SANDBOX.md`（per_user 桌面）、`docs/plans/media/VIDEO_ATOMIZATION_PLAN.md`（视频计费口径来源）。
...
> `docs/operations/RELAY_CUTOVER.md` §5），到期语义随之变成：不续费 = 关自动续费，桌面到期后由
...
> `docs/operations/RELAY_CUTOVER.md`。本节原文保留作历史，"ueejavelin"处按操作单理解。
