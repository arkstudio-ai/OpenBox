...
- **M1 后端：已完成（2026-09-11）**，实现细节与接口见 [MESSAGE_CENTER.md](../../architecture/MESSAGE_CENTER.md)。与本方案的差异：迁移 id 为 `a1c2e3b4d5f6`（`f1a2b3c4d5e6` 已被占用）；授权失效保留"工作空间广播 + 发起者个人"两条不同粒度记录，不合并；超管预览发我复用扇出逻辑而非测试台延迟。
- **M3 App：已完成（2026-09-11）**，见 [MESSAGE_CENTER.md](../../architecture/MESSAGE_CENTER.md)「App 端」。与本方案的差异：链接解析器放在 `features/inbox/state/inbox_navigator.dart`（feature 层不反向依赖 app 层，router 由调用方传入）；点击推送以 `POST /api/inbox/{id}/read` 的响应作为 link 来源，不再单独拉记录；未加 AppBar 铃铛，入口只在抽屉。
- **M4 后台界面：已完成（2026-09-11）**，见 [MESSAGE_CENTER.md](../../architecture/MESSAGE_CENTER.md)「后台界面」。与本方案的差异：App 端没有把公告塞进「通知测试」栏，而是加了第五个底部入口「消息通知」，避免与现有测试台的滚动结构互相干扰；Web 的确认框在 `admin-messages` 内自带一份（feature 之间不互相引用）。
- **M2 Web 用户侧：已完成（2026-09-11）**，见 [MESSAGE_CENTER.md](../../architecture/MESSAGE_CENTER.md)「Web 用户侧」。PR #20 早于本分支基线合入，铃铛直接放在合并后的侧栏。与本方案的差异：传输层放在 `shared/api/inbox.ts`（侧栏属 workspace feature，不能跨 feature 引用）；授权中心条幅未切新接口（定位不同，见文档）。
- M5 上线：未开始。
