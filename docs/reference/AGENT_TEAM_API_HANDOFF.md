...
2026-09-22。本文覆盖接口、后端和 Web；移动端沿用同一接口契约，界面优化不在本次范围。实际请求字段以运行服务的 `/openapi.json` 为准。
...
客户端应固定勾选 `core_tools`，不能通过预设或自定义勾选取消；服务端解析 Agent 定义时也会
补齐这 10 项。新团队默认的 `delegable_tools` 同样包含核心工具；显式授权和历史运行快照
保持原范围，不能因为应用新默认值恢复已撤销的能力。

核心工具的执行环境由会话所属工作空间解析，普通会话、成员会话和试运行共用沙箱连接。
试运行的普通命令与文件操作沿用主会话默认执行权限，显式部署规则和敏感路径仍需按规则确认；
不会因为定义编译器生成的通用 `ask` 规则卡住。团队成员仍受本次运行已确认的操作范围约束，
权限不足返回 `PERMISSION_REQUIRES_USER`，不能误报成“没有沙箱”。本地共享无影模式下，
热点等桌面集成同样使用执行端的配置，不要求另有工作空间云桌面数据库记录。

技能目录的 `allowed_tools`、`requires_mcp` 是显式绑定技能时的依赖声明。客户端自动合并并
锁定对应的工具和 MCP；依赖 MCP 的工具范围为 `["*"]`，不可单独删掉或改窄。服务端编译和
保存时补齐依赖，缺失、不可委派或超出运行授权会返回现有业务错误，不能只在页面勾选后忽略。
只启用“全部可用”发现模式时不自动请求所有技能依赖；已有运行的冻结权限不会被回填扩张。
