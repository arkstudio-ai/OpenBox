...
> 2026-09-07 新增，见 `docs/plans/media/A5_AUTHORIZATION_CENTER.md`。除回调与 Webhook 外都要带 `Authorization` 与 `X-Workspace-Id`。
...
| GET / POST | `/api/inbox?category=&unread=&cursor=&limit=`、`/api/inbox/unread`、`/api/inbox/{id}/read`、`/api/inbox/read-all` | 成员 | 消息中心：跨工作空间收件箱、分类未读数、已读（见 [MESSAGE_CENTER.md](../architecture/MESSAGE_CENTER.md)） |
...
> 2026-09-08 新增，见 `docs/plans/product/ADMIN_CONSOLE_SKILL_STORE_PLAN.md` §4.5.1。实现在 `backend/api/admin_skills.py`，决策逻辑在 `backend/skill/user_library.py`。
...
> 2026-09-08 新增，见 `docs/plans/product/ADMIN_CONSOLE_SKILL_STORE_PLAN.md` §4.5.2。实现在 `backend/api/admin_billing.py`。
