...
范围更新（2026-09-21 晚）：用户解除移动端暂停，要求按 Web 实现与第 13 节把原生界面做完并在 iOS 模拟器上实测。原生团队界面已重做并通过 453 项移动端测试；模拟器上完成了一次真实运行的完整链路（输入区 → 组队方案卡 → 进度卡 → 完成与最终答复）以及运行详情、成员只读会话和工作面板入口。侧边栏此前缺少“Agent 团队”入口，现已补上，并配套我的 Agent / 团队模板 / 运行记录三个页签（编辑仍在 Web，第 13.6 节），模板运行与“再运行一次”均已在模拟器上验证。未在模拟器上实测的部分：运行中的暂停/继续/取消、含真实文件成果的“成果”页，以及设备上的深色与英文界面（这三项以渲染图和自动化测试为证据）。详见 [实现记录](AGENT_TEAM_IMPLEMENTATION.md)。
...
自动化模块均位于 `backend/tests/unit/test_team_*.py`；子进程验证脚本位于 `backend/scripts/`。最新计数和运行 ID 见 [实现记录](AGENT_TEAM_IMPLEMENTATION.md)，恢复步骤见 [运维说明](../../operations/AGENT_TEAM_OPERATIONS.md)。
