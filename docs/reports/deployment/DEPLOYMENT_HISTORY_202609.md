# 部署历史与旧版操作说明（2026 年 9 月）

> 归档于 2026-09-22。下文是当时的发布记录与旧操作说明，价格、镜像、迁移版本及“当前”均指对应记录时点。
> 新部署请从[现行部署指南](../../operations/DEPLOY.md)与 [gw2 Runbook](../../../deploy/gw2/RUNBOOK.md)进入；本文件不代表现网状态。

> 工具目录说明（2026-09-22）：当前实现已按职责迁移，见[工具目录](../../reference/TOOLS.md)。
> 本文历史发布章节保留当时镜像中的文件路径，排查当前源码请使用新的职责路径。
...
- 代码 PR [#52](https://github.com/arkstudio-ai/OpenBox/pull/52)，修复提交 `17294c2`，合并到 `main@afa9d69f`；构建源码与该 main 完全一致。快照 HTTP 413 改为分批事件回放；已知图片素材在录制入队前变为引用，由 worker 校验归属、去重和处理删除。模型请求保持原样，历史缺口保留。原因和边界见 [Trace 修复说明](../../operations/TRACE-LARGE-INPUT-REPLAY.md)。
...
- 配套 App：Android `1.0.23+34` 与 iOS `1.0.23 (34)`（TestFlight 已上传处理完成），见 [MOBILE_RELEASE_1_0_23_20260916.md](../mobile/MOBILE_RELEASE_1_0_23_20260916.md)。
...
- 83 项测试通过，覆盖过期链接、临时 403/503、持久化退避、8 次 / 24 小时上限、聊天内轮询、跨进程恢复与不重复提交生成请求。策略与旧视频可恢复性的限制见 [视频转存恢复](../../operations/VIDEO_TRANSFER_RECOVERY.md)。
...
- 配套 iOS **1.0.21 (32)** 已于同日 08:53 上传 TestFlight，Apple 处理完成且现有“运营测试组”可安装；连接本生产后端。最低系统为 iOS 15，移动端不新增管理端 trace 回放页，详见 [发布记录](../mobile/MOBILE_RELEASE_1_0_21_20260916.md)。此次 iOS 打包未改动后端或录制范围。
...
- 07:31（上海时区）复核：五个容器 OOM=false、自动重启次数为 0；业务/worker 自 07:23 起无 ERROR 或 traceback。worker 已接收 97 个事件、上传 6 个 blob，写入失败/冲突/丢弃/隔离均为 0，投影积压为 0，业务库 trace SQL 仍为 0。348 次公网探测中有 11 次切换期 502 和 1 次客户端 8 秒超时（未取得 HTTP 响应，无法仅据此归因服务端）；超时后约 8 分钟探测均为 200。
- 07:38–07:46 原生 iOS 生产验收：`1.0.20+31` 完成 SSO、三轮聊天、后台恢复、冷启动登录恢复、已有 1,347 轮会话的历史分页及五栏超管只读检查。验证会话 3 对消息，trace 提交/投影 125/125，28 次历史请求全部 200，业务 trace SQL 仍为 0。此前 SSO 失败已定位为模拟器把密码首字母改成大写，保持原始大小写即登录成功。详见 [移动验收记录](../mobile/MOBILE_RELEASE_1_0_20_20260916.md)。
- **09:12 已全量开启录制**：按用户确认，保留 `TRAJECTORY_RECORDING_ENABLED=true`，清空 `TRAJECTORY_RECORD_USER_IDS`，依次重新创建 worker 和 backend；两次切换前活动执行租约均为 0。配置文件及运行中容器回读一致，后端实际录制开关检查覆盖现有 26/26 用户及未来新用户；管理员查看权限保持原值。只记录开启后产生的事件，不补录此前未录制的历史。
- 全量开启前配置备份位于 `/opt/openbox/backups/trace-all-users-20260916T011149Z/backend.env`（目录 0700、文件 0600）。worker / backend 均 healthy，worker 的 writer / db / spool / blob_store 均 true，公网返回 200；切换后即时检查入库失败、blob 上传失败、隔离文件和投影积压均为 0。业务库约 146 MB、trace 库约 18 MB。将这两项录制设置恢复为备份中的值，并在无活动执行时依次重新创建这两个服务，可恢复指定内部账号录制范围。
- 09:13 复核：录制范围内已有事件的用户数从 1 增至 2，会话从 2 增至 3，事件从 1,332 增至 1,335，确认原白名单外的新事件进入 trace；业务库 trace SQL 仍为 0。后端和 worker 自切换完成起无 ERROR 或 traceback，投影积压为 0。单次 CPU 采样 backend 为 9.25%、worker 为 85.19%（Docker 按一个核为 100%），仅用于记录当时状态，不代表并发性能结论。
- 本次是配置扩围与健康检查，尚未完成运行手册要求的等长 ≥60 分钟生产性能比较，不能作为 50 并发真实模型的容量证明。此前原生 iOS 功能验收与本次录制范围验证的含义不同。
...
- 源码 `main@73a311b` 全量构建，两边同一 tag，**gw2 与 AWS 自此回到可追溯的 main sha**，此前 gw2 的三层叠加热修镜像（714a30b / 6f90252 / 6aee878+68bb0fa）全部已在 main 内。
  ...
  [#35](https://github.com/arkstudio-ai/OpenBox/pull/35) 历史分页后端与 nginx gzip、`0b99bf1`。移动端同日发 Android `1.0.19+30` 与 iOS `1.0.19 (30)`（已上传 TestFlight，待点出口合规；见 [MOBILE_RELEASE_1_0_19_20260915.md](../mobile/MOBILE_RELEASE_1_0_19_20260915.md)）。
...
- [排查、修复、发布与待办](../mobile/ANDROID_BACKGROUND_PUSH_20260911.md)，[发布证据](../../evidence/android-background-push-20260911.json)。AWS 本次未发布。
...
- 内容：侧栏第一行改回「新对话」（落当前选中项目），「新建项目」降为第二行；logo 可点回 `/app`；六个中心行进入时高亮；
  ...
  规划见 `docs/plans/product/FRONTEND_NAV_PROFILE_PLAN.md`。
...
- 切换期间首页/API 各 107 次采样、10 次 502，约 10.8 秒后恢复且后续持续 200；
  详情及仓库已有移动端门禁问题见 [追加 QA 记录](../media/VIDEO_LAYOUT_AND_FRONTEND_RECOVERY_QA_20260909.md)。
...
- 最终切换 2 分钟采样：首页/API 各 109 次，其中 10 次 502、99 次 200；失败至恢复约 10.8 秒，之后持续正常。
  ...
  [视频排版与前端恢复 QA](../media/VIDEO_LAYOUT_AND_FRONTEND_RECOVERY_QA_20260909.md)。
...
- 内容：`video_compose` 平台原子工具（自有时间线 → IMS Timeline 编译层，六条规则）、IMS 客户端、补扫、
  ...
  技能第 8 步双路径 + 第四张「合成确认」卡。详见 `docs/research/VIDEO_RENDER_ENGINE_SELECTION.md`。
...
- 备份 `/opt/openbox/backups/20260909-qwh-recovery/`，镜像包 `/opt/openbox/releases/20260909-qwh-56ef0f8/`。详细因果、逐项验收及回滚限制见 [QWH 修复报告](../platform/QWH_BROWSER_RECOVERY_20260909.md)。
...
- 新增管理端 Skill / MCP CRUD、回收站、ZIP 多选批量上传、批量删除与图标编辑；
  ...
  业务边界与测试详情见 [技能管理验证说明](../platform/ADMIN_SKILL_MANAGEMENT_QA.md)。
...
- 详见 `docs/plans/media/A5_DESKTOP_LOGIN_STATE.md` §7.6。
...
- 内容：探针分层（transport/connect/http/parse）、失败自动采集桌面诊断快照并在工具错误里引用
  ...
  详见 `docs/plans/platform/D1_ECD_BROWSER_DEBUG.md` §7 与 `docs/operations/WUYING_SANDBOX.md`「Reading a broken desktop」。
...
- 15/15 台（含共享桌面与 prewarm）`/opt/openbox/skills/{video-production,douyin-publish}` 已同步到 `origin/main` 版本；旧目录备份在各桌面 `/opt/openbox/backups/`。下发方式见 `docs/plans/media/A5_AUTHORIZATION_CENTER.md` §9.5。
...
> 轨迹 worker 拓扑（§五）上线后，AWS 开发机的 `config/backend.env` 必须设 `TRAJECTORY_OSS_INTERNAL=false`：worker 默认走 OSS 内网端点，
> 该端点只在阿里云内可达，AWS 上访问不到。
...
## 五、会话轨迹 worker 拓扑与发布流程（轨迹改造，尚未上线）

> 状态：随 `feat/trajectory-rearch` 准备（SPEC §12），**尚未在任何环境发布**；上线后照惯例在本文开头追加发布记录。
> 命令、校验、回滚、告警与演练的完整步骤见 [deploy/gw2/RUNBOOK.md](../../../deploy/gw2/RUNBOOK.md)，设计见 [trajectory-rearch/SPEC.md](../../trajectory-rearch/SPEC.md)。
...
浏览器 → 反向代理 → frontend 容器(nginx)
                      ...
                      ├── /api/、/ws/                                        → backend:8080
                      └── /                                                  静态 SPA
backend ──(追加 JSONL)──► trajectory-spool 卷（只挂给 backend 与 worker）──► trajectory-worker
trajectory-worker ──► postgres 的 openbox_trace 库、OSS <桶>/trajectories/、redis、backend 内部接口（viewer / audit）
宿主机 systemd 定时器 ──► 云监控自定义指标、OSS backups/postgres/、镜像清理
```
...
- **新服务 `trajectory-worker`**：与 backend 同一镜像，override 里与 backend 钉同一 tag。命令只跑轨迹库迁移
  ...
  并把 `DATABASE_URL` 置空，`backend.env` 里的业务库连接串不会进入 worker。worker 默认走 OSS 内网端点；AWS 开发环境访问不到该端点，
  那里的 worker 必须设 `TRAJECTORY_OSS_INTERNAL=false`（写在 `config/backend.env`）。
- **backend**：`TRAJECTORY_WORKER_MODE=external`，挂 spool 卷；业务请求只做内存入队与追加写，业务库不再有轨迹读写。
  `TRAJECTORY_WORKER_MODE=off` 时整条轨迹链路关闭：不启动 emitter 与元数据同步，也没有 worker。
...
0. 前置：**发布提交先合入 `origin/main`**。否则其他人从 main 例行发布会让 backend 起不来：main 构建的镜像在迁移脚本里找不到
   `d3b5f7a9c1e2`、`e5c7a9b1d3f4` 两个业务迁移版本，`alembic upgrade head` 失败，backend 无法启动。
...
6. `backend.env` 先设 `TRAJECTORY_RECORDING_ENABLED=false`，override 改 backend 镜像，0 活动运行时 `up -d --no-deps backend`；
   业务迁移删除 7 张旧轨迹表，旧录制不保留。核对业务库已没有旧轨迹表、`alembic current` 为 `e5c7a9b1d3f4`（命令见 RUNBOOK §5），
   随后立即在 `openbox` 执行 `SELECT pg_stat_statements_reset();`：迁移的 `DROP TABLE` 语句会命中 `business_trajectory_statements` 的隔离过滤。
7. 删除旧录制的 payload 文件：业务 `blob-data` 卷（backend 内挂在 `/tmp/openbox-blobs`）只删 `trajectories/` 目录（约 1.5 GB，未脱敏），
   保留 `policies/`；先列出大小再删除，不可恢复，命令见 RUNBOOK §5 第 7 步。
...
- **backend（第 6 步及以后）**，0 活动运行时依次：
  1. 用**新** backend 镜像执行 `docker compose run --rm --no-deps backend alembic downgrade c7e9b1d3f5a7`，重建 7 张旧轨迹表（空表；
     main 构建镜像的就绪检查与会话删除仍依赖这些表）；
  2. `config/backend.env` 保持 `TRAJECTORY_RECORDING_ENABLED`、`TRAJECTORY_ADMIN_ENABLED` 为 false（第 9 步已开启录制时先改回，再做下一步）；
  3. override 把 backend、frontend 改回旧 tag，逐个 `up -d --no-deps` 并等 healthy；
  4. `docker compose stop trajectory-worker`，再从 `.env` 的 `COMPOSE_FILE` 去掉 overlay。

  旧录制在升级与回退两个方向都不保留：重建的旧表是空的，写入 `openbox_trace` 的轨迹旧后台也看不到。
...
| 项 | 内容 |
|---|---|
| `openbox-trajectory-metrics.timer` | 每分钟：宿主机磁盘、spool 大小与最老文件年龄、近 1 小时 OOM kill、轨迹库大小、backend 的 CPU（单核百分比）与内存（占上限百分比，均取自 `docker stats --no-stream`）、业务库 `pg_stat_statements` 中含 `trajectory_` 的语句数（未装扩展时为 0），加上 worker 的 `/health`、`/metrics`（含 wave 3 新指标），经云监控 `PutCustomMetric` 上报（维度 `instance=gw2`；其他主机如 AWS 开发机要用 `install-timers.sh --instance <名称>` 安装，否则会触发 gw2 的告警） |
| `openbox-pg-backup.timer` | 每天 03:30（北京时间）：`openbox`、`openbox_trace` 的 `pg_dump -Fc` 经预签名 PUT 上传到 OSS `backups/postgres/<日期>/`，校验大小与 sha256 后删除本地文件；旧录制不保留，`openbox` 的 dump 用 `--exclude-table-data` 跳过旧轨迹表的数据（只留表结构，迁移删表后不再匹配任何表）；PostgreSQL 查询失败、导出或上传失败都会让本次运行失败；单次 PUT 上限 5 GiB |
| `openbox-prune-images.timer` | 每周日 04:30（北京时间）：保留容器在用、compose 引用及每个仓库最新 3 个 tag 的镜像，其余删除；读不出 compose 配置时不删除任何镜像 |
| 告警（`setup-alarms.sh`，联系人组 `云账号报警联系人`，共 15 条） | 磁盘 ≥ 80%、spool ≥ 1 GiB、最老 spool 文件 ≥ 60 s、worker 健康检查失败、近 1 小时出现录制缺口、投影积压 ≥ 5000 事件、5 分钟内 blob 上传失败 ≥ 10、轨迹库 ≥ 20 GiB、OOM kill > 0、归档积压 ≥ 50000 事件持续 15 分钟、`hot_partitions` > 10（已挂载且日期不晚于今天的每日事件分区数，按 UTC，不含预建的未来分区与默认分区，健康时约 8 个）、backend CPU 或内存 > 90% 持续 5 分钟、业务库出现 `trajectory_` 语句、`events_ingested_24h` > 1,000,000（近 24 小时摄取的事件数，每 5 分钟采样，INFO）；`--instance`、`--group-id` 要与指标上报一致 |
