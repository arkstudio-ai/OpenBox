...
日期：2026-09-09；2026-09-16 更新底部主按钮交互，验证结果见文末。范围：React Web（桌面 / 手机网页）与 Flutter 客户端。
仅使用本地测试账号、Docker 数据库及已授权的无影开发配置；未操作其他用户会话，未部署线上。

后续阿里云 Docker 正式发布已于同日完成，见 [部署记录](../../operations/DEPLOY.md)。本文保留分页开发阶段的测试证据与边界。
...
- 底部主按钮在非最后一题显示“下一题”，当前题有有效答案后可点击，点击只翻页。
  多选题可连续勾选或取消选项，选完点底部“下一题”；全部取消后按钮禁用。
  顶部分页仍可浏览未答题，也可以回到前题检查或修改答案。
- 底部主按钮仅在最后一题显示“确认”；即使全组已经答完，回到前题仍显示“下一题”。
  最后一题（含多选）不越界，也不自动提交。所有题都回答后才可点一次“确认”，按原题序提交完整答案。
...
Web 与 Flutter 的底部主按钮按当前页显示“下一题”或“确认”。中间页只要求当前题有有效答案，
最后一页要求全组完整；多选不自动翻页，取消所有选项后不能继续，回看已答题也不会提前提交。

本次本地验证：

- Web `QuestionDock.test.tsx`：29 项通过；改动文件 ESLint 与 `tsc -b` 通过。
- Chromium `playwright.questions.config.ts`：17 项通过，覆盖 320 / 390 / 1280px、多选连续操作与完整提交。
- Flutter `question_dock_test.dart`、`question_scroll_test.dart`：40 项通过；包含中间题与末题多选、
  自填下一题、回看修改、草稿恢复、故障重试，以及既有 iOS / Android widget 滚动回归。

本次仅修改与验证本地代码，未部署，也未进行实体设备验收。
