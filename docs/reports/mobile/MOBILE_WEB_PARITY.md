...
> 性质：跨端对齐与验收记录，按日期保留能力、依赖和已验证范围。2026-09-21 起移动端优先对接接口，UI 优化暂停，后续设计单独安排。
>
...
1. Web 新增跨端能力时，更新接口契约和本文的差异记录。当前范围只要求移动接口接入，界面优化不随 Web 变更自动扩展。
...
2026-09-21 最新接入范围：用户明确“移动只要对接接口就行了，我会设计的”。[团队接口说明](../../reference/AGENT_TEAM_API_HANDOFF.md) 记录请求、分页、状态、幂等、作用域和错误处理约定；移动接口专项 3 项测试通过。完整四组长评测也已停止，改为 API / Web 的随机抽样与故障修复。

2026-09-21：Agent Team 原生移植按用户要求**暂停优化**，本轮优先完成接口、后端和 Web。工作区已保留团队模式与模板选择、方案详情、进度卡、任务/消息/成果/用量页、成员只读会话和按需 WS 订阅。iOS 本地账号实测了模板选择、完整最终总结、任务展开与重新展开、成员只读及返回团队、工作面板入口；最近全量 432 项测试通过，随后任务详情修复通过 3 项专项测试。后续原生新建运行、控制与断网的设备端验收，以及最终修订的双端重新打包暂不继续；不把窄屏 Web 验收当作原生验收。

2026-09-18：Agent 重构分支的上下文优化展示已同步至原生端，见下方「上下文优化与流式恢复」。
...
### 上下文优化与流式恢复（2026-09-18）

- 原生端将手动和自动压缩合并进当前回合的处理区域，默认折叠，显示进行中、完成、失败和中断；摘要不进入普通回答、工作日志或回复操作。
- `agent=compaction` 在摘要流出第一段文字前即可识别；旧历史兼容 `summary=true`。`auto`、`replacement_id` 和 `summary` 的流式更新、历史快照比较已补齐。
- Web / Flutter 都保留已经落定的压缩状态，较早发起的历史请求不会把完成状态退回进行中。连续压缩按请求 ID 分组，只有真实用户消息开始新回合；内部压缩不会强制滚动历史阅读位置。
- `/api/agent/config` 的模型条目新增可选 `compaction` 描述：`enabled`、默认输入 `threshold`、各推理档位的 `variants` 阈值。后端复用运行时的阈值和输出预留计算；两端随模型/推理档位选择显示，旧服务端缺字段或关闭自动压缩时不承诺会自动压缩。
- 同步 Web 中英文 locale，包含此前遗漏的管理端和账单文案；压缩算法与 80% 默认值由后端负责，客户端不截断模型上下文。

回归入口：Flutter `compaction_*_test.dart`、`context_ring_test.dart`；Web `playwright.compaction.config.ts`；后端 `test_compaction_metadata.py`。浏览器和原生组件验收使用隔离数据，不调用线上模型或云桌面。
...
参考：[B1 工作空间审计](../../plans/platform/B1_WORKSPACE_AUDIT.md)。
...
参考：[桌面执行通道](../../plans/platform/A1_DESKTOP_CHANNEL.md) 与 [舰队/预热池](../../plans/platform/A3_A4_FLEET_POOL.md)。
...
参考：[计费方案](../../plans/product/BILLING_PLAN.md)。
...
来源：`docs/plans/product/ADMIN_CONSOLE_SKILL_STORE_PLAN.md`，分支 `codex/admin-console`。这一轮把技能商店从「第三方目录 + 即发即上架」改成「官方技能 + 经审核的用户投稿」，并把 Web 的「舰队管理」升级成「超管系统」。
...
完整配置、交互与测试命令见 [下一步建议说明](../../architecture/next-step-suggestions.md)。
...
11. **Android Logto 单一回调**：邀请链接只保留生产 HTTPS App Link；删除 MainActivity 上缺少 host 的自定义 scheme 过滤器。Android 在过滤器未声明 host 时会忽略 `pathPrefix`，旧配置因此同时匹配 `com.bossip.bipmobile://callback`，形成两个授权跳转入口。2026-09-09 起仅由 `.AuthCallbackActivity` 接收：仍交给 SDK 验证 OAuth，并显式恢复原 App task，修复授权成功后 Chrome 仍覆盖 App 的问题（详见 [Logto 配置与回跳契约](../../operations/LOGTO_PROD.md#android-callback-task-restoration-1016--2026-09-09)）。
...
| 日期 | 功能块 | 提交 | 验收证据 |
|---|---|---|---|
| 2026-09-16 | iOS 1.0.21 (32) TestFlight 发布 | 归档源码 `c3c4897` | analyze、Release 归档、分发签名、Apple 校验/上传通过；Apple 状态 `VALID` / `IN_BETA_TESTING`，现有“运营测试组”已关联构建，中文测试说明已保存。Xcode 27 要求最低 iOS 15；不增加管理端 trace 页面。见 [发布记录](MOBILE_RELEASE_1_0_21_20260916.md) |
| 2026-09-16 | 1.0.20+31 聊天可靠性更新与 gw2 联调 | 基线 `08f5d952` | 381 项测试、拆分后 24 项历史回归、analyze、locale/800 行门禁与 iOS 构建通过；生产 SSO、三轮聊天、后台/冷启动恢复、1,347 轮历史分页和原生管理入口通过。125 个事件全部投影，28 次历史读取全为 200。移动端不新增管理端 trace 页面。见 [验收记录](MOBILE_RELEASE_1_0_20_20260916.md) |
...
| 2026-09-10 | 双端通知 API 与单手机登录 | `codex/mobile-notifications`，未部署 | APNs/极光桥接与设置入口、最新手机登录生效、通知换绑与投递队列；PostgreSQL 迁移及多进程并发测试、Flutter 回归、Android debug/iOS simulator 构建。已接主任务终态、待处理、定时结果及可信发布回调；明确 resumed/inactive/后台/超时离线边界、前台抑制与取消队列。真机远程送达及 Android 厂商通道仍待验收，见 [移动通知说明](../../reference/MOBILE_NOTIFICATIONS.md) |
