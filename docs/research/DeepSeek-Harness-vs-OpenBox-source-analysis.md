...
> 文档类型：研究 / 选型。源码引用与结论受调研版本约束，外部仓库引用不是本项目运行依赖。
...
本文来自 `codex/agent-refactor` 的历史源码分析。下文保留了 2026-08-31 的比较证据，
其中“当前”“已实现”及部署验收描述均指当时的重构分支，不能作为 2026-09-17
整合分支的实现或验收结论。

本次以 main 为基准移植 Driver/Inbox、canonical events、工具调度、权限、子任务与
扩展生命周期的设计，同时保留 main 已有的 workspace、每用户云桌面、问题恢复、
计费、通知、视频、Cron 和 trajectory。没有移植旧共享桌面、远端 lease receipt、
容器 MCP supervisor 或 Cron lease/outbox 改造。

当前实现以 [Agent 架构](../architecture/AGENT_KERNEL_ARCHITECTURE.md) 和
[整合记录](../reports/agent/AGENT_MAIN_INTEGRATION.md) 为准；本次没有验证历史文档中的云上环境。
...
DeepSeek Harness 是 DeepSeek 官方 MIT 开源项目，当前仓库位于 `/Volumes/fanxiang/workspace2/deepseek-harness`。官方 README 明确说明它仍处于开发者预览，未来会发生破坏兼容性的变更：README.zh.md（外部源码参考：`deepseek-harness/README.zh.md:11`）。安全说明进一步声明它没有完成安全审计，不得视作安全或生产就绪软件：SAFETY.zh.md（外部源码参考：`deepseek-harness/SAFETY.zh.md:5`）。
...
DeepSeek Harness 的模型适配器、工具注册表、Session 日志、Agent Loop、沙箱、审批、Web Host 都是 Cordis 插件。插件向 Context 提供 Service、监听事件并登记可逆 Effect；不存在需要直接打补丁的特权核心：architecture.zh.md（外部源码参考：`deepseek-harness/docs/architecture.zh.md:9`）。
...
源码入口：context.ts（外部源码参考：`deepseek-harness/vendor/cordis/src/context.ts:35`）、reflect.ts（外部源码参考：`deepseek-harness/vendor/cordis/src/reflect.ts:127`）。

每个插件统一实例化为 Fiber。Fiber 状态机覆盖 `PENDING → LOADING → ACTIVE/FAILED → UNLOADING → DISPOSED`；函数、类和对象插件最终都进入相同生命周期：registry.ts（外部源码参考：`deepseek-harness/vendor/cordis/src/registry.ts:91`）、fiber.ts（外部源码参考：`deepseek-harness/vendor/cordis/src/fiber.ts:139`）。
...
源码证据：fiber.ts（外部源码参考：`deepseek-harness/vendor/cordis/src/fiber.ts:402`）、fiber.ts（外部源码参考：`deepseek-harness/vendor/cordis/src/fiber.ts:597`）。
...
主要证据：profile.ts（外部源码参考：`deepseek-harness/packages/boot/app-boot/src/profile.ts:1`）、profile-boot.ts（外部源码参考：`deepseek-harness/apps/cli/src/profile-boot.ts:105`）、app-boot/index.ts（外部源码参考：`deepseek-harness/packages/boot/app-boot/src/index.ts:742`）、plugin.ts（外部源码参考：`deepseek-harness/apps/cli/src/plugin.ts:30`）。
...
证据：entry.ts（外部源码参考：`deepseek-harness/vendor/loader/src/config/entry.ts:141`）、group.ts（外部源码参考：`deepseek-harness/vendor/loader/src/config/group.ts:59`）。

Include 支持 YAML `!!js`、按 id 的 insert/override、串行 apply、配置刷新和 `.tmp + rename` 原子写入：include/index.ts（外部源码参考：`deepseek-harness/vendor/include/src/index.ts:43`）。
...
`live` Profile 才会监听用户 patch；`startup` Profile 启动后冻结。Watch-only fallback 只能刷新配置，不能刷新源码；源码 HMR 还依赖 Node loader internals，回滚属于 best effort。profile-boot.ts（外部源码参考：`deepseek-harness/apps/cli/src/profile-boot.ts:264`）、hmr/index.ts（外部源码参考：`deepseek-harness/vendor/hmr/src/index.ts:199`）。
...
证据：agent-presets/index.ts（外部源码参考：`deepseek-harness/packages/preset/agent-presets/src/index.ts:380`）、agent-presets/index.ts（外部源码参考：`deepseek-harness/packages/preset/agent-presets/src/index.ts:746`）。

## 5. OpenBox 的启动与扩展架构

OpenBox 由 FastAPI lifespan 手工初始化基础设施、Agent、数据库、Sandbox reconcile、Redis Bus、Cron 和恢复任务；关闭时手工中断 Loop、停止 Cron、关闭 Bus，并保留可恢复的 Provider 资源：[main.py](../../backend/main.py)。
...
证据：[registry.py](../../backend/tool/registry.py)、[registry.py](../../backend/tool/registry.py)、[registry.py](../../backend/tool/registry.py)。
...
源码：agent.ts（外部源码参考：`deepseek-harness/packages/core/agent-loop/src/agent.ts:219`）、agent.ts（外部源码参考：`deepseek-harness/packages/core/agent-loop/src/agent.ts:234`）、agent.ts（外部源码参考：`deepseek-harness/packages/core/agent-loop/src/agent.ts:254`）。
...
入口证据：[sessions.py](../../backend/api/sessions.py)、[loop.py](../../backend/agent/loop.py)、[processor.py](../../backend/agent/processor.py)。
...
当前模型返回的完整 tool-call batch 会先做 call-id 冲突和重复校验，这是一个优点：任何歧义在副作用前整批 fail closed。[processor.py](../../backend/agent/processor.py)

校验完成后，顶层工具调用通过 `for` 循环逐个执行：[processor.py](../../backend/agent/processor.py)。只有模型显式调用 `batch` 工具时才会并行子调用：[batch.py](../../backend/tool/collaboration/batch.py)。而 `batch` 内部直接调用 `tool.execute`，虽然再次校验可见性、`parallel_safe` 和授权，但没有为每个嵌套工具完整经过 ToolHooks 的 running/completed/error/trace 生命周期。
...
证据：[sessions.py](../../backend/api/sessions.py)、[loop.py](../../backend/agent/loop.py)。

Abort registry 是进程内字典，新 Run 会覆盖同 Session 的当前槽位：[status.py](../../backend/session/status.py)。这会导致：
...
OpenBox 还会先把 Session 设置成 `IDLE`，再 fire-and-forget 地扫描 pending/running ToolPart 并标记 interrupted：[loop.py](../../backend/agent/loop.py)、[loop.py](../../backend/agent/loop.py)。新 Prompt 可以在旧清理执行前启动，因此旧清理有误伤新 Run ToolPart 的风险。
...
架构说明：architecture.zh.md（外部源码参考：`deepseek-harness/docs/architecture.zh.md:49`）、surface.ts（外部源码参考：`deepseek-harness/packages/core/session/src/surface.ts:1`）、session/index.ts（外部源码参考：`deepseek-harness/packages/core/session/src/index.ts:567`）。

这使“用户曾经看到的历史”“模型当前看到的上下文”“压缩替代关系”“审计与重放”能够同时成立，而不需要真的改写或删除旧消息。

### 7.2 DeepSeek Harness 的持久化屏障与崩溃修复

持久化插件可以 write-behind 批量写入，但 Agent Loop 会在关键外部副作用前显式 `flush`：例如下一次模型请求、工具副作用以及 turn 关闭前。这样既保留批处理效率，也提供顺序与错误观察屏障。session-checkpoint-policy（外部源码参考：`deepseek-harness/packages/session/session-checkpoint-policy/src/index.ts:20`）
...
证据：repair.ts（外部源码参考：`deepseek-harness/packages/core/session/src/repair.ts:28`）。
...
[`MEDIA_EFFECT_SAFETY.md`](../architecture/MEDIA_EFFECT_SAFETY.md)。
...
这是很实用的产品 CRUD 模型，也是 OpenBox 刷新后能恢复文本和 Tool 卡片的基础。[db/base.py](../../backend/db/base.py)
...
证据：[session.py](../../backend/session/session.py)、[session.py](../../backend/session/session.py)、[session.py](../../backend/session/session.py)。
...
源码：[session.py](../../backend/session/session.py)。

主 Loop、Compaction、Fork、Abort 等调用没有覆盖这个 limit，例如主 Loop 直接调用 `get_messages(session_id)`：[loop.py](../../backend/agent/loop.py)。因此超过 200 条 Message 后，查询继续返回**最老的 200 条**，最新用户输入可能完全不进入：
...
入口：[loop.py](../../backend/agent/loop.py)、[loop.py](../../backend/agent/loop.py)。

这里有一个执行面错位：instruction loader 从 Backend 宿主 `Path.cwd()` 和宿主 OS 读取 AGENTS/Instruction 文件，而项目实际位于独立 Docker/Kubernetes/Wuying Sandbox 中。因此 Sandbox 项目里的指令文件通常不在 Backend 宿主路径，Agent 可能无法看到真实项目的 AGENTS.md。[instruction.py](../../backend/session/instruction.py)、[instruction.py](../../backend/session/instruction.py)
...
证据：region.ts（外部源码参考：`deepseek-harness/packages/compaction/compaction-basic/src/region.ts:92`）、region.ts（外部源码参考：`deepseek-harness/packages/compaction/compaction-basic/src/region.ts:138`）、region.ts（外部源码参考：`deepseek-harness/packages/compaction/compaction-basic/src/region.ts:340`）、region.ts（外部源码参考：`deepseek-harness/packages/compaction/compaction-basic/src/region.ts:436`）。
...
证据：[agent/compaction.py](../../backend/agent/compaction.py)、[agent/compaction.py](../../backend/agent/compaction.py)、[agent/compaction.py](../../backend/agent/compaction.py)、[agent/compaction.py](../../backend/agent/compaction.py)。
...
读取逻辑：[session/compaction.py](../../backend/session/compaction.py)。

**结论：OpenBox 的 Compaction 产品体验和超长历史 fallback 很实用；Harness 的正确性、可审计性和事务边界明显更好。**

### 7.7 Fork、Regenerate 与 Revert

Harness Fork 只复制已经完成 turn 的稳定日志前缀，拒绝在开放 turn 中间 Fork，并保留事件语义和 provenance：session/index.ts（外部源码参考：`deepseek-harness/packages/core/session/src/index.ts:1065`）。
...
证据：[fork.py](../../backend/session/fork.py)、[fork.py](../../backend/session/fork.py)、[project/workspace.py](../../backend/project/workspace.py)。

OpenBox Regenerate 是硬删除后续 Transcript，不可审计：[session.py](../../backend/session/session.py)。文件 Revert 则基于 Project 级 Git tree snapshot，不是 Session branch；恢复会改写整个共享 Project 目录。`unrevert` token 只在进程内字典保存，重启即丢：[snapshot.py](../../backend/snapshot/snapshot.py)、[snapshot.py](../../backend/snapshot/snapshot.py)、[revert.py](../../backend/session/revert.py)。

准确描述应是“对话复制 + Project 文件快照恢复”，而不是耐久的 Git 式 Session 分支。

### 7.8 OpenBox 的恢复、Retry 与 Cache 边界

OpenBox 正常结束时会扫描 pending/running ToolPart 并标记 interrupted；优雅 shutdown 会给活跃 Loop 发 abort，然后把仍处于 busy/compacting 的 Session 标成 error。[loop.py](../../backend/agent/loop.py)、[main.py](../../backend/main.py)
...
证据：[processor.py](../../backend/agent/processor.py)、[processor.py](../../backend/agent/processor.py)、[llm.py](../../backend/agent/llm.py)、[llm.py](../../backend/agent/llm.py)。现有相关单测 mock generator 直接抛异常，没有覆盖实际 Adapter 的 error-event 语义。
...
证据：[caching.py](../../backend/agent/caching.py)、[loop.py](../../backend/agent/loop.py)。
...
策略 Listener 可以 deny 或 ask；Guard 仍可施加最终拒绝，已拒绝的调用不能被后续 Listener 重新放宽。工具参数、审计记录、UI 和真实执行保持同一份不可变含义。tools README（外部源码参考：`deepseek-harness/packages/core/tools/README.md:83`）
...
证据：[tool_runtime.py](../../backend/agent/tool_runtime.py)、[tool_exposure.py](../../backend/agent/tool_exposure.py)、[tool_exposure.py](../../backend/agent/tool_exposure.py)。
...
定义层通过 Pydantic Schema 验证参数并统一截断输出：[hooks.py](../../backend/agent/hooks.py)、[tool.py](../../backend/tool/tool.py)。
...
证据：[loop.py](../../backend/agent/loop.py)。这意味着 `bash cat .env` 不会命中 `read *.env`，而是走默认 Bash allow。

规则采用 last-match-wins：[permission.py](../../backend/permission/permission.py)。`ask()` 将用户历史“always allow”规则追加到 config+agent 规则之后，因此用户持久化 allow 有覆盖 Agent deny 的潜在风险：[permission.py](../../backend/permission/permission.py)、[hooks.py](../../backend/agent/hooks.py)。Plan Agent 只明确 deny edit-family 工具，仍保留 Bash，所谓只读很大程度依赖提示词：[agent.py](../../backend/agent/agent.py)。
...
配置证据：base cordis.patch.yml（外部源码参考：`deepseek-harness/packages/bundle/base/cordis.patch.yml:205`）。
...
源码声明：sandbox/index.ts（外部源码参考：`deepseek-harness/packages/sandbox/sandbox/src/index.ts:23`）。安全说明也明确要求不要将其作为不可信工作负载的唯一安全控制，建议在一次性 VM、容器或专用环境中运行：SAFETY.zh.md（外部源码参考：`deepseek-harness/SAFETY.zh.md:11`）。
...
证据：[manager.py](../../backend/sandbox/manager.py)、[manager.py](../../backend/sandbox/manager.py)、[manager.py](../../backend/sandbox/manager.py)、[docker.py](../../backend/sandbox/docker.py)。
...
证据：[container/Dockerfile](../../container/Dockerfile)、[container/Dockerfile](../../container/Dockerfile)、[action_server.py](../../container/action_server.py)、[action_server.py](../../container/action_server.py)、[action_server.py](../../container/action_server.py)、[docker.py](../../backend/sandbox/docker.py)。
...
动态创建的 `V1PodSpec` 有 PVC、资源 requests/limits、readiness/liveness probe 和可配置 ServiceAccount，这是正面基础：[kubernetes.py](../../backend/sandbox/kubernetes.py)、[kubernetes.py](../../backend/sandbox/kubernetes.py)。Owner metadata 校验和确定性资源冲突处理也比较完整。
...
证据：[kubernetes.py](../../backend/sandbox/kubernetes.py)。仓库中的 `sandbox-pod-template.yaml` 虽写过 runAs 1000，但动态创建代码并不读取该模板：sandbox-pod-template.yaml（历史引用，当前仓库未收录：`k8s/sandbox-pod-template.yaml`）。

### 9.5 Control Plane 与 Preview 风险

本地 Compose 中 Backend 挂载 Docker socket，Backend 进程一旦失陷，通常等价于宿主高权限：[docker-compose.yml](../../docker-compose.yml)。这不是 Sandbox 容器直接拿到 Docker socket，但要求对 Backend 自身和所有宿主 Custom Tool 采用高信任模型。
...
证据：[containers.py](../../backend/api/containers.py)、[containers.py](../../backend/api/containers.py)、[provider.py](../../backend/sandbox/provider.py)。

`ContainerInfo` 数据模型还直接包含 `api_key` 字段，API 返回模型需要逐个确认是否有脱敏：[container.py](../../backend/models/container.py)。Refresh Cookie 当前 `secure=False`，公网 HTTPS 部署前也必须修复：[auth/routes.py](../../backend/auth/routes.py)。
...
证据：skill/index.ts（外部源码参考：`deepseek-harness/packages/skill/skill/src/index.ts:248`）、skill/index.ts（外部源码参考：`deepseek-harness/packages/skill/skill/src/index.ts:328`）、skill/index.ts（外部源码参考：`deepseek-harness/packages/skill/skill/src/index.ts:521`）。

Filesystem Provider 的层次包括 Project `.dsh`、Project `.agents`、Custom、User `.dsh`、User `.agents` 和 Bundled。只识别 Root 下 `*.md` 或一层目录中的 `SKILL.md`，Frontmatter 必须包含 name、description；非 Bundled 内容通过挂载的 `ctx.fs` 读取。Watcher 和 Cache 都有明确上限。skill-filesystem/index.ts（外部源码参考：`deepseek-harness/packages/skill/skill-filesystem/src/index.ts:36`）、skill-filesystem/index.ts（外部源码参考：`deepseek-harness/packages/skill/skill-filesystem/src/index.ts:658`）、skill-filesystem/index.ts（外部源码参考：`deepseek-harness/packages/skill/skill-filesystem/src/index.ts:793`）。

模型入口只先看摘要，正文只有精确调用后才进入上下文，避免所有 Skill 全文占满 Prompt：tool-skill/index.ts（外部源码参考：`deepseek-harness/packages/skill/tool-skill/src/index.ts:71`）、tool-skill/index.ts（外部源码参考：`deepseek-harness/packages/skill/tool-skill/src/index.ts:206`）。
...
证据：[skill.py](../../backend/skill/skill.py)、[skill.py](../../backend/skill/skill.py)、[skill_tool.py](../../backend/tool/knowledge/skill_tool.py)、[skill_tool.py](../../backend/tool/knowledge/skill_tool.py)。
...
证据：[skill_manage.py](../../backend/tool/knowledge/skill_manage.py)、[action_server.py](../../container/action_server.py)、[action_server.py](../../container/action_server.py)。
...
支持 stdio 和 Streamable HTTP。Stdio 使用 scrubbed parent environment + 显式 env：transport.ts（外部源码参考：`deepseek-harness/packages/mcp/mcp-client/src/transport.ts:15`）。
...
证据：connection.ts（外部源码参考：`deepseek-harness/packages/mcp/mcp-client/src/connection.ts:172`）、connection.ts（外部源码参考：`deepseek-harness/packages/mcp/mcp-client/src/connection.ts:227`）、connection.ts（外部源码参考：`deepseek-harness/packages/mcp/mcp-client/src/connection.ts:307`）。

工具同步先抓完所有分页，确认新 Generation 完整后再整体替换旧 Generation；注册冲突会撤销新注册，避免部分新、部分旧的混合状态：tools.ts（外部源码参考：`deepseek-harness/packages/mcp/mcp-client/src/tools.ts:120`）。公开名称有长度/字符约束和稳定 Hash，MCP `isError` 转异常；Rich Image 只有在格式、附件存储和当前模型 image input 能力全部满足时才持久化，否则降级为诊断文本。
...
证据：mcp-client README（外部源码参考：`deepseek-harness/packages/mcp/mcp-client/README.md:10`）、mcp-client README（外部源码参考：`deepseek-harness/packages/mcp/mcp-client/README.md:184`）。
...
证据：[action_server.py](../../container/action_server.py)、[mcp_tool.py](../../backend/tool/integrations/mcp_tool.py)、[mcp_tool.py](../../backend/tool/integrations/mcp_tool.py)、[mcp_tool.py](../../backend/tool/integrations/mcp_tool.py)。
...
证据：[action_server.py](../../container/action_server.py)、[action_server.py](../../container/action_server.py)、[action_server.py](../../container/action_server.py)、[action_server.py](../../container/action_server.py)。
...
每个版本不可变，Stable Plugin 保留 package map、approvals、current/next/latest。一般 inspect 不返回源码，只有 exact package inspect 才能读取对应 source。registry.ts（外部源码参考：`deepseek-harness/packages/extensions/cordis-host-runner/src/registry.ts:16`）
...
证据：cordis-host-runner/index.ts（外部源码参考：`deepseek-harness/packages/extensions/cordis-host-runner/src/index.ts:146`）、cordis-host-runner/index.ts（外部源码参考：`deepseek-harness/packages/extensions/cordis-host-runner/src/index.ts:768`）、cordis-host-runner/index.ts（外部源码参考：`deepseek-harness/packages/extensions/cordis-host-runner/src/index.ts:1247`）。
...
证据：sandbox.ts（外部源码参考：`deepseek-harness/packages/extensions/cordis-host-runner/src/sandbox.ts:1`）、sandbox.ts（外部源码参考：`deepseek-harness/packages/extensions/cordis-host-runner/src/sandbox.ts:243`）、guard.ts（外部源码参考：`deepseek-harness/packages/extensions/cordis-host-runner/src/guard.ts:490`）、agent.cordis.yml（外部源码参考：`deepseek-harness/packages/preset/agent-presets/presets/cordis/agent.cordis.yml:1`）。
...
Provider 必须显式声明支持的 capability；请求了不支持的 model/reasoning/persona/tool filter/output schema 等能力会 fail loud。subagent types（外部源码参考：`deepseek-harness/packages/subagent/subagent/src/types.ts:75`）、subagent runtime（外部源码参考：`deepseek-harness/packages/subagent/subagent/src/index.ts:535`）
...
证据：child-agent.ts（外部源码参考：`deepseek-harness/packages/subagent/subagent/src/child-agent.ts:39`）、child-agent.ts（外部源码参考：`deepseek-harness/packages/subagent/subagent/src/child-agent.ts:166`）。
...
证据：continuation.ts（外部源码参考：`deepseek-harness/packages/subagent/subagent/src/continuation.ts:395`）、continuation.ts（外部源码参考：`deepseek-harness/packages/subagent/subagent/src/continuation.ts:942`）、continuation.ts（外部源码参考：`deepseek-harness/packages/subagent/subagent/src/continuation.ts:1167`）。
...
FastAPI 路由装配：[main.py](../../backend/main.py)。Lifespan 会初始化 DB、Redis、Sandbox reconcile、Cron 和恢复，并在关闭时中断活跃 Loop、关闭 Bus、保留可恢复的 Provider 资源：[main.py](../../backend/main.py)。

这是 OpenBox 相对 Harness 最明显的优势：它不是只有 Agent 内核，而是已经有完整业务控制面。

### 14.2 WebSocket 与 Redis Bus

主 WebSocket 使用一次性 Ticket，按 user 路由实时事件；前端有单飞连接、代际防旧 Socket 覆盖和指数退避。[ws.py](../../backend/api/ws.py)、[frontend ws client](../../frontend-v2/src/shared/ws/client.ts)
...
证据：[bus.py](../../backend/bus/bus.py)、[bus.py](../../backend/bus/bus.py)。WebSocket 队列有上限，拥塞时会丢非关键事件；重连依靠 DB Snapshot 和前端重新拉取收敛，而不是 Event Replay。[ws.py](../../backend/api/ws.py)
...
证据：[cron model](../../backend/db/models/cron.py)、[timer.py](../../backend/cron/timer.py)、[timer.py](../../backend/cron/timer.py)、[executor.py](../../backend/cron/executor.py)、[recovery.py](../../backend/cron/recovery.py)。
...
证据：[package.json](../../frontend-v2/package.json)、[router.tsx](../../frontend-v2/src/app/router/router.tsx)、[useChatEvents.ts](../../frontend-v2/src/features/chat/hooks/useChatEvents.ts)。
...
证据：[Makefile](../../Makefile)、[docker-compose.yml](../../docker-compose.yml)、[frontend-v2 Dockerfile](../../frontend-v2/Dockerfile)、gke.md（历史引用，当前仓库未收录：`gke.md`）。
...
入口：ci.yml（外部源码参考：`deepseek-harness/.github/workflows/ci.yml:21`）、e2e.yml（外部源码参考：`deepseek-harness/.github/workflows/e2e.yml:54`）、package.json（外部源码参考：`deepseek-harness/package.json:35`）。

它还保留真实故障 Postmortem，包括“178 个单测和 100% 行覆盖仍未发现真实编辑器加载路径崩溃”的案例。这种记录说明其团队意识到覆盖率不能替代真实装配路径验证：postmortem 0001（外部源码参考：`deepseek-harness/docs/postmortem/0001-acp-default-export-drops-inject.zh.md:13`）。

不过，工程 Gate 更完整不改变其官方“开发者预览、破坏兼容、未安全审计”的状态。

### 15.3 OpenBox 文档与源码的关键不一致

1. README 称“Pydantic AI 单轮工具循环”，实际运行时没有创建或执行 `pydantic_ai.Agent`，而是自研 Processor + LiteLLM/Responses。`_to_pydantic_messages()` 只是未接入的兼容 Helper：[README.md](../../README.md)、[llm.py](../../backend/agent/llm.py)、[llm.py](../../backend/agent/llm.py)。
2. README 称“每 Session 独占 Sandbox”，实际是每 User 一个容器、每 Project 一个共享目录：[README.md](../../README.md)、[manager.py](../../backend/sandbox/manager.py)。
3. README/设计文档称 MCP 运行在宿主，实际 MCP 在 Sandbox action server 内：[README.md](../../README.md)、[action_server.py](../../container/action_server.py)。
4. README 称已部署 GKE，GKE 文档末尾仍把真实集群联调列为待执行事项：[README.md](../../README.md)、gke.md（历史引用，当前仓库未收录：`gke.md`）。
