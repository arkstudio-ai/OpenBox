...
分析日期：2026-09-10。本文保留迁移前的源码分析，表格中的“新版现状”指迁移前基线。后续已按用户确认实现通知 API、双端接入和单手机登录；最终接口、配置和验证见 [MOBILE_NOTIFICATIONS.md](../reference/MOBILE_NOTIFICATIONS.md)。尚未部署。
...
初始标识来自旧版接入记录（外部源码参考：`bossip/apps/codex/v2/mobile/PUSH_NOTIFICATION_ENABLEMENT.md`）和 Gradle/Xcode 配置。后续按用户要求从旧服务端提取了 APNs 私钥和极光 Master Secret，并修正 Key ID；私密内容仅进入本地忽略配置，没有写入文档或 Git。尚未向推送服务验证凭据是否被撤销。
...
- 旧版没有完整的国内 Android 厂商离线通道。新版后续已加入七个官方厂商适配器、配置文件及正式签名入口；2026-09-18 已启用小米客户端配置，其余厂商按需填写。见 [Android 厂商推送配置](../operations/ANDROID_PUSH_VENDORS.md)。普通后台、最近任务划掉、系统强制停止分别验收。
...
| 依据 | 文件 |
| --- | --- |
| 旧版接入及测试记录 | PUSH_NOTIFICATION_ENABLEMENT.md（外部源码参考：`bossip/apps/codex/v2/mobile/PUSH_NOTIFICATION_ENABLEMENT.md`） |
| 旧 Flutter 平台接口 | system_notifications.dart（外部源码参考：`bossip/apps/codex/v2/mobile/lib/src/core/system_notifications.dart`） |
| 旧通知展示与点击策略 | codex_notification_coordinator.dart（外部源码参考：`bossip/apps/codex/v2/mobile/lib/src/core/codex_notification_coordinator.dart`） |
| 旧发送状态机 | push-runtime.ts（外部源码参考：`bossip/apps/codex/v2/apps/api/src/services/push-runtime.ts`） |
| 旧绑定、去重及投递存储 | push-store.ts（外部源码参考：`bossip/apps/codex/v2/apps/api/src/services/push-store.ts`） |
| 新执行结算 | [loop.py](../../backend/agent/loop.py)、[runtime.py](../../backend/question/runtime.py) |
| 新执行和问题持久化 | [question.py](../../backend/db/models/question.py) |
| 新事件总线 | [bus.py](../../backend/bus/bus.py) |
| 新认证退出 | [routes.py](../../backend/auth/routes.py)、[auth_store.dart](../../mobile/lib/shared/api/auth_store.dart) |
| 新工作空间恢复与路由 | [workspace_bootstrap.dart](../../mobile/lib/app/workspace_bootstrap.dart)、[router.dart](../../mobile/lib/app/router.dart) |
| 新原生入口 | [AppDelegate.swift](../../mobile/ios/Runner/AppDelegate.swift)、[AndroidManifest.xml](../../mobile/android/app/src/main/AndroidManifest.xml) |
