...
- Writer thread: waits on a condition (≤ 100 ms), assigns `n`, builds the line by byte concatenation (`b'{"v":1,"k":"event","n":' + n + b',"t":"' + t + b'","event":' + event_json + b'}\n'`), writes to the open `.part` file (buffered), flushes buffers at least every 100 ms, rotates when size ≥ `file_bytes` or age ≥ `file_ms` (non-empty): flush, `fsync`, rename `.part` → `.jsonl`. While more than 100 closed files of its own wait in its producer directory (`Emitter.BACKLOG_FILES`, counted with the producer-directory rescan below, its own closes added in between) the worker is behind, and the writer rotates by size or at age 30 s (`Emitter.BACKLOG_FILE_SECONDS`, or `file_ms` when that is longer) instead: an outage leaves a few large files rather than one per second of activity, which the worker drains at its big-file rate. `file_ms` rules again once the backlog is at or below the threshold.
...
- Provider request bodies are recorded verbatim, so a prompt can be debugged from its trace. `request_snapshot` keeps the complete body a provider call sends, at any depth (an SDK model as its JSON dump), and leaves out only the top-level transport settings and credentials in `TRANSPORT_FIELDS` (`api_key`, `api_base`/`base_url`, `organization`, `headers`/`extra_headers`/`default_headers`, `extra_query`, `timeout`, `max_retries`, `client`/`http_client`, `authorization`, `cookie(s)`, `access_token`/`refresh_token`/`security_token`, `credential(s)`), naming them in `omitted_fields`. A service dispatch body (`capture_service_dispatch`) follows the same rule, with each retained media URL replaced by its asset reference. `public_value` still drops underscore keys and `PRIVATE_FIELDS` from tool arguments, metadata and usage.
- `request.delta` keeps the provider chunk verbatim: `raw` is `provider_output_snapshot(chunk)`, the SDK model's `model_dump(mode="json")` or the mapping as received, with no field allowlist, no copy and no stream references, next to the adapter's `blocks`. Events are encoded when recorded (§5.3), so a later change by the adapter cannot alter a recorded chunk, and only a value JSON cannot hold becomes a `not_recorded` marker (§5.4). `job.progress` records `provider_response` the same way. There is no end-of-stream recorder event.
- Tool output (`trajectory/tool_output.py`): each call has one `ToolOutputStream`. Cumulative pushes (`ToolContext.update_output`) and bash's appended chunks past its chat budget become `tool.output` records with `stage: "executor_stream"`: the new text as `mode: "delta"`, rewritten text as `mode: "replace"`. A call makes at most one per `TOOL_OUTPUT_RECORD_SECONDS` (1 s): the first change is recorded at once, later changes are coalesced and a timer records them at the end of the interval; the chat bus still gets every push. Streamed records stop at `TRAJECTORY_TOOL_OUTPUT_MAX_BYTES` for the call (§13), the last one with `stream_truncated: true`. The retained full output is recorded once at the end (`stage: "executor_result"`, `final: true`) as the call's last `tool.output`: closing the stream first cancels a pending flush and waits for one in flight. It keeps the whole output when it fits the cap, otherwise the first and last half of the cap joined by a marker line stating the omitted bytes, with `output_bytes`, `output_sha256` (computed off the event loop above 1 MiB) and `output_truncated: true`. `tool.finished` carries the model-facing `model_output`.
- File versions (`trajectory/files.py`) are encoded once each; their digests and the unified diff are computed with `asyncio.to_thread` when a version may exceed 64 KiB, and two versions over 4 MiB together are recorded with `diff: null` and `diff_skipped: "too_large"`. The versions and the worker (§8.4) change nothing but U+0000.
...
- Checkpoints: when `projected_seq - checkpoint_seq >= TRAJECTORY_CHECKPOINT_INTERVAL`, capture records with a keyset scan and an 8 MiB stored/decoded read budget per group. Stream expanded canonical records to a bounded temporary file (512 MiB total, 16 MiB per record), then hash the existing full-state format and upload one page at a time (at most 100 records and a 4 MiB target, except a single larger record). Pages remain content-addressed and unchanged pages dedupe. Insert the checkpoint row and advance `checkpoint_seq` in a short transaction. A moved projection retries at a new watermark; a size limit or the 120 s deadline backs off checkpoint creation without stopping ingest/projection. `TRAJECTORY_CHECKPOINT_{READ_BYTES,PAGE_BYTES,RECORD_BYTES,MAX_BYTES,TIMEOUT_SECONDS}` configure these bounds.
...
- `get_session_header`, `_metadata`: same outputs; identity fields from meta tables; recording flags evaluated from env (shared). A live probe (no `through_seq`) whose summary row is at `projected_seq` reads no record data and replays nothing, even while projection lags the head: statistics, agents and unsupported events are the summary row and record summaries as of `projected_seq` (`statistics.through_seq` and `projected_through_seq` say so); `running_status` and `model` are carried from the summary through the hot events after `projected_seq` up to the head whose type family is `run`, `permission` or `question`, or that are `request.started` (seq order, `LIMIT 2000`), with the projection's summary rules. A tail that fills that limit, a record written past `projected_seq` or a projection that commits during the reads (archival may then move tail rows) falls back to the replay. An explicit `through_seq` is served from summaries only at `projected_seq` and replayed otherwise.
- `read_events`: seq ≤ `archived_seq` from segments (download + zstd decode + sha256 verify; in-memory LRU of decoded segments, `TRAJECTORY_SEGMENT_CACHE_BYTES` 32 MiB), seq > `archived_seq` from hot rows; gap and tail checks unchanged; `include_data=true` expands `$ref` (always) and `$payload` (as today) with `visible_references`; blob fetches use a concurrent fetcher (≤16 in flight) and an LRU cache of decoded blobs (`TRAJECTORY_BLOB_CACHE_BYTES` 64 MiB).
...
- Admin reads (`store/database.py` `trace_read_session`): every SELECT runs in its own short read-only transaction on a separate pool (`TRAJECTORY_READ_DB_POOL_SIZE`, no overflow, 1 s wait; `statement_timeout` 5 s, `lock_timeout` 1 s, `idle_in_transaction_session_timeout` 5 s, `work_mem` 4 MB, no parallel workers), so no connection is held while objects are fetched or decoded; the fixed watermark still bounds the read. SQLite (embedded) uses the writer's engine. One JSON read loads at most `TRAJECTORY_READ_MAX_BYTES` (8 MiB, the response cap) of decoded blobs and segments, cached content included (`read_budget.py`), and fails with 413 beyond it (§8.12); background work has no read budget.
...
- Read limits (`worker/read_limits.py`, every admin GET route): at most `TRAJECTORY_READ_CONCURRENCY` (2) reads run and `TRAJECTORY_READ_MAX_WAITING` (4) wait up to `TRAJECTORY_READ_WAIT_MS` (250); more get 429 `trajectory_read_busy` with `Retry-After: 1`. A JSON read must be ready within `TRAJECTORY_READ_TIMEOUT_SECONDS` (10), a payload, blob or export download within `TRAJECTORY_DOWNLOAD_PREPARE_TIMEOUT_SECONDS` (60); a missed deadline or a PostgreSQL statement or lock timeout gets 503 `trajectory_read_timeout` with `Retry-After: 1`. The decoded-content budget (§8.9) or a JSON body over `TRAJECTORY_READ_RESPONSE_BYTES` (8 MiB) gets 413 `trajectory_read_too_large`, and the viewer halves its event page, keeping cursor and watermark. A JSON response releases its slot before it is sent. A download has no decoded budget; once its content is spooled and revalidated it gives its read slot back and streams under one of `TRAJECTORY_READ_TRANSFERS` (4) transfer slots until the stream ends or the client goes away, so slow clients hold transfer slots, never the read slots JSON reads need; when no transfer slot frees up within `TRAJECTORY_READ_WAIT_MS` its spooled content is released and it gets the same 429 `trajectory_read_busy`. A WebSocket subscription reads only the trajectory row for its watermark (never the session header), outside the admission and with at most 2 connections at once, which the read pool keeps beyond the HTTP slots. A socket checks its viewer (account, token expiry and revocation, mobile session) once a second: a send or a received message within a second of the last check reuses it, and an otherwise idle socket is checked every second, so a revocation closes it within about a second. Health, metrics, tickets and the socket never wait for a read slot. The viewer's event stream (`frontend-v2/src/features/admin-trajectories/api/sync.ts`) reads for a watermark hint at once when its last live event read ended 300 ms ago or more and otherwise 300 ms after that read; hints that arrive during a read add exactly one more read; a live read answered 429 or 503 is retried after its `Retry-After` (1 s without one) instead of failing, and neither hints nor polls read before then.
...
- `GET /metrics` (JSON, admin network only; not routed by nginx): counters `ingest_lines`, `ingest_events`, `duplicates`, `idempotency_conflicts`, `deleted_drops`, `ownership_drops`, `gaps_recorded`, `producer_loss_events`, `quarantined_files`, `blob_puts`, `blob_put_bytes`, `blob_put_failures`, `segment_uploads`, `segment_failures`, `gc_deleted`, `gc_failures`, `exports_built`; gauges `spool_bytes`, `spool_files`, `spool_oldest_age_seconds`, `ingest_lag_seconds`, `projection_lag_events`, `archive_lag_events`, `gc_queue_depth`, `trace_db_bytes` (sampled every 5 min), `hot_events_rows`, `trajectories_degraded`, `trajectories_blocked`, `stale_hot_partitions`; spool usage by area, gauges `spool_blob_bytes`, `spool_quarantine_bytes`, `spool_quarantine_files` (`spool_bytes` also counts the producer directories); read limits (§8.12), counters `read_rejected`, `read_timed_out`, `read_too_large` and gauges `read_active`, `read_waiting`, `read_transfers` (downloads streaming under a transfer slot).
...
| `TRAJECTORY_SPOOL_FILE_MS` | 1000 (30 s while more than 100 closed files of the producer wait, §5.2) | backend |
...
| `TRAJECTORY_TOOL_OUTPUT_MAX_BYTES` | 262144 (UTF-8 bytes of one tool call's output: its streamed `tool.output` records together, and its final `executor_result`, which above it keeps the first and last half with `output_bytes`, `output_sha256` and `output_truncated`; §5.5) | backend |
...
| `TRAJECTORY_SEGMENT_CACHE_BYTES` / `TRAJECTORY_BLOB_CACHE_BYTES` | 33554432 / 67108864 (cached content, not a process memory limit) | worker |
| `TRAJECTORY_READ_CONCURRENCY` / `TRAJECTORY_READ_MAX_WAITING` / `TRAJECTORY_READ_WAIT_MS` | 2 / 4 / 250 (admin GET reads running, waiting, longest wait; beyond them 429 with `Retry-After: 1`, §8.12) | worker |
| `TRAJECTORY_READ_TRANSFERS` | 4 (payload, blob and export downloads streaming to clients after their content was spooled and their read slot released; a download that finds none free within `TRAJECTORY_READ_WAIT_MS` gets 429, §8.12) | worker |
...
| `TRAJECTORY_READ_DB_POOL_SIZE` | `TRAJECTORY_READ_CONCURRENCY` + 2 (read-only pool of admin reads: one connection per HTTP read slot and two for WebSocket subscription watermarks; no overflow, 1 s wait) | worker |
...
| `OPENBOX_TRACE_DATABASE_URL` | unset: the overlay uses `openbox_trace` on the shared postgres; a dedicated PostgreSQL instance's `postgresql+asyncpg://…` URL keeps trace CPU, memory and IO off the business database (host backup and metric scripts still target the shared instance) | overlay |
