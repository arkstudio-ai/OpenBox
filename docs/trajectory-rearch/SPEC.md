...
- Orphan sweep (every 10 min, catches objects no row ever referenced): list the next 1000 keys of the namespace after the cursor in `trajectory_worker_state` `gc.orphan_cursor` (back to the start after a short page); objects older than 7 days that the GC re-check finds unused (no available payload row for a blob, no segment row for a segment, no live export row for an export; anything else in a trajectory directory) and that have no GC `key` entry get one with reason `orphan_object`, decided in batched queries. Prefix entries are never queued; the daily report counts `gc_orphans_queued`.
