...
- `scripts/create-trace-db.sh`: idempotent `CREATE DATABASE openbox_trace`, `CREATE EXTENSION IF NOT EXISTS pg_trgm` and `pg_stat_statements` where appropriate.
...
- `scripts/push-metrics.sh`: host disk usage, spool size/oldest age, worker `/metrics`, backend OOM kill count (`journalctl -k`), trace DB size → `python -m trajectory.ops.cms push` executed inside the worker container.
- systemd units + timers: `openbox-trajectory-metrics.timer` (every minute), `openbox-pg-backup.timer` (daily 03:30 Asia/Shanghai), `openbox-prune-images.timer` (weekly); installed by `scripts/install-timers.sh`.
...
- `scripts/setup-alarms.sh`: CloudMonitor rules (contact group `云账号报警联系人`) for host disk ≥ 80 %, spool bytes ≥ 1 GiB, spool oldest age ≥ 60 s, worker health down, gaps per hour > 0, projection lag ≥ 5000, blob put failures ≥ 10 / 5 min, trace DB ≥ 20 GiB, OOM kill > 0.
- Drills: `drill-worker-stop.sh`, `drill-blob-outage.sh` (worker env `TRAJECTORY_BLOB_FAULT=put:1.0` for N minutes), `drill-spool-full.sh` (backend `TRAJECTORY_SPOOL_MAX_BYTES` temporarily tiny), `rebuild-trace-db.sh` (restore segments into a scratch database and compare digests).
