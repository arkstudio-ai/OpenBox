import { ChatReadOnlyContext } from "../contexts/read-only"
...
export function ComposerAccess({
  ...
  return (
    <div className="border-hair text-n600 border-t px-5 py-3 text-center text-sm">
      {t(memberSession ? "readOnlyTeamSession" : "readOnlySession")}
    </div>
  )
}
...
function WaitingForInput({ status, onCancel }: { status?: SessionStatus; onCancel: () => void }) {
  ...
  return (
    <div
      className="text-n600 mx-auto flex w-full max-w-190 items-center justify-between px-4 py-2 text-xs"
      role="status"
    >
...
export interface ChatSurfaceProps {
  sessionId: string
  resourceScope?: import("../hooks/useMentionMenu").MentionScope
  ComposerComponent?: ComponentType<import("./Composer").ComposerProps>
...
function composerSessionProps(session: Session | undefined, agents: ChatAgent[] | undefined) {
  const fixedConfiguration = session?.kind === "agent_trial"
  return {
    fixedConfiguration,
    sessionModel: session?.model,
    sessionVariant: session?.variant,
    sessionVideoModel: session?.video_model,
    sessionVideoResolution: session?.video_resolution,
    contextTokens: session?.token_usage?.context ?? 0,
    contextLimit: session?.token_usage?.limit ?? 0,
    agents: fixedConfiguration ? [{ name: session.agent ?? "build" }] : (agents ?? EMPTY_AGENTS),
  }
}
...
export function ChatSurface({
  ...
  useEffect(() => {
    if (questionsQ.data?.items)
      usePendingStore.getState().setQuestions(questionsQ.data.items, questionsQ.data.read)
  }, [questionsQ.data])
  useEffect(() => {
  ...
  const readOnly =
    forcedReadOnly ||
    session.data?.kind === "team_member" ||
    isReadOnlySession(session.data?.user_id, currentUserId)
  ...
  const footer = (
    <>
      {footerSlot}
      {!readOnly && permissions.map((p) => <PermissionCard key={p.id} request={p} />)}
      {!readOnly && questions.map((q) => <QuestionDock key={q.id} request={q} />)}
    </>
  )

  return (
    <ChatReadOnlyContext value={readOnly}>
      <div className="flex h-full min-h-0 flex-col">
        ...
        {loading ? (
          ...
          <ChatFlow
            ...
            footer={footer}
            renderTurnTools={renderTurnTools}
        ...
        {!readOnly && <WaitingForInput status={recoveredStatus} onCancel={stop} />}
        {runError && (
          <RunErrorNotice
            message={runError}
        ...
        <ComposerAccess readOnly={readOnly} memberSession={session.data?.kind === "team_member"}>
          <ComposerComponent
            {...composerSessionProps(session.data, agents)}
            busy={busy}
