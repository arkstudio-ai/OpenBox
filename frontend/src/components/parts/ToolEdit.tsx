...
export function ToolEdit({ input, output }: ToolEditProps) {
  ...
  const safeOutput = projectScopedToolText(output || "");
      ...
      <div className="px-3.5 py-2.5 space-y-0.5 overflow-x-auto max-h-64 overflow-y-auto bg-[hsl(var(--terminal-bg))]">
        {oldStr &&
          oldStr.split("\n").map((line, i) => (
            <div
              key={`old-${i}`}
              className="flex bg-[hsl(var(--destructive))]/8 text-[hsl(var(--destructive))] rounded-sm px-1.5 py-px"
            >
              <span className="text-[hsl(var(--destructive))]/50 mr-2.5 select-none">
                -
              </span>
              <span>{line}</span>
            </div>
        ...
        {newStr &&
          newStr.split("\n").map((line, i) => (
            <div
              key={`new-${i}`}
              className="flex bg-[hsl(var(--success))]/8 text-[hsl(var(--success))] rounded-sm px-1.5 py-px"
            >
              <span className="text-[hsl(var(--success))]/50 mr-2.5 select-none">
                +
              </span>
              <span>{line}</span>
            </div>
          ))}
        {safeOutput && !oldStr && !newStr && (
