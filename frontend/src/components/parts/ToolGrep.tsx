...
export function ToolGrep({ input, output }: ToolGrepProps) {
  ...
  const path = projectScopedDisplayPath(String(input?.path || ""));
  const lines = projectScopedDisplayText(output || "")
    .split("\n")
    .filter(Boolean);
    ...
    <div className="font-mono text-[11px]">
      <div className="px-3.5 py-2 bg-[hsl(var(--surface-1))] text-[hsl(var(--muted-foreground))]">
        ...
        {path && (
          <>
            ...
            <span className="text-[hsl(var(--accent))]">{path}</span>
          </>
        )}
      </div>
      <div className="px-3.5 py-2.5 space-y-0.5 overflow-x-auto max-h-48 overflow-y-auto bg-[hsl(var(--terminal-bg))]">
        {lines.slice(0, 30).map((line, i) => (
          <div
            key={i}
            className="text-[hsl(var(--foreground))] hover:bg-[hsl(var(--surface-1))] rounded-sm px-1.5 py-0.5 cursor-pointer transition-colors"
          >
            {line}
          </div>
        ))}
